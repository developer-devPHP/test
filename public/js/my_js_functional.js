				...
				onSelect : function(selectedDate)
				{
					...
					if (dateObject)
					{
						...
						if ($('#check_out_date').val() != '')
						{
							...
							var chech_out_date_val = $.datepicker.parseDate(
									'yy-mm-dd', $('#check_out_date').val());

							date_caunt = Math.ceil((chech_out_date_val
									.getTime() - new_day.getTime())
									/ (one_day)) + 1;
						...
						onSelect : function(selectedDate)
						{
							...
							if (dateObject)
							{
								...
								if ($('#check_in_date').val() != '')
								{
									...
									var chech_in_date_val = $.datepicker
											.parseDate('yy-mm-dd', $(
													'#check_in_date').val());

									date_caunt = Math
											.ceil((new_date.getTime() - chech_in_date_val
													.getTime())
													/ (one_day)) + 1;

									$('#number_of_nights').val(date_caunt);
									$('#number_of_nights').removeAttr(
											'disabled');
			...
			.live(
					...
					function()
					{
						...
						if ($(this).val().length !== 0)
						{
							...
							{
								if ($("#dialog-message-invalid_date").dialog(
										"isOpen"))
								{
									$('body')
											.append(
													'<div id="dialog-message-invalid_date" title="Invalid Date">ashjkdasdh</div>');
								}
								...
								$('#number_of_nights').attr("disabled",
										"disabled");
								$('#number_of_nights').val(null).attr(
										'selecdet', 'selected');
							}
						}
						else
	...
	$('form input:submit')
			.on(
					...
					function()
					{
						...
						$('body')
								.append(
										"<div class='ui-widget-overlay' id='preloader'></div>");
						$('#preloader').css(
						{
							'width' : $(window).width(),
