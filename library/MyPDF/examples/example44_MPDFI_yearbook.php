...
// First write all your entries to a PDF file, forcing each entry to fit on one
// page
include ("../mpdf.php");

// Define the maximum containing box width & height for each text box as it will
// appear on the final page (no padding or margin here)
$pw = 80;
$ph = 110;
$minK = 0.7; // Maximum scaling factor 0.7 = 70%
$inc = 0.01; // Increment to change scaling factor 0.05 = 5%
$spacing = 10; // millimetres (vertically and horizonatlly between boxes in
               // output) shrinks if boxes too big
...
$mpdf = new mPDF('', array(
        ($pw * (1 / $minK)),
        ($ph * (1 / $minK))
), '', '', 0, ($pw * (1 / $minK)) - $pw, 0, ($ph * (1 / $minK)) - $ph, 0, 0);

$pph = array();

// FOR EACH ENTRY FOR YOUR YEARBOOK saving the page height in $pph (where $html
// is the HTML code for the entry):
...
for ($i = 1; $i <= 10; $i ++)
{
    ...
    $pph[$i] = SinglePage($html, $pw, $ph, $minK); // $pph saves the actual
                                                    // height of each page
}
// ==============================================================
...
for ($i = 1; $i <= $pagecount; $i ++)
{
    ...
    $x2 = $mpdf->lMargin + $mpdf->pgwidth / 2 + $hspacing / 2 +
             ($mpdf->pgwidth / 2 - $hspacing / 2 - $pw) / 2;
    $y1 = $mpdf->tMargin + ($pgheight / 2 - $vspacing / 2 - $ph) / 2;
    $y2 = $mpdf->tMargin + $pgheight / 2 + $vspacing / 2 +
             ($pgheight / 2 - $vspacing / 2 - $ph) / 2;
    if ($i % 4 == 1)
    {
        $x = $x1;
        $y = $y1;
    ...
    {
        $mpdf->UseTemplate($tplIdx, $x, ($y + (($ph - $pph[$i]) / 2)), $pw, 
                $pph[$i]);
    }
    
    if ($border >= 0)
    {
        $mpdf->Rect($x - $border, $y - $border, $pw + 2 * $border, 
                $ph + 2 * $border);
    }
}
...
function SinglePage ($html, $pw, $ph, $minK = 1, $inc = 0.1)
{
    ...
    while ($currpage != $newpage)
    {
        ...
        $mpdf->SetFont($mpdf->default_font, '', $mpdf->default_font_size, true, 
                true); // forces
                                                                                            // write
        $mpdf->SetDrawColor(0);
