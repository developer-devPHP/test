...
if ($handle = opendir(_MPDF_PATH . 'graph_cache'))
{
    ...
    {
        if (((filemtime(_MPDF_PATH . 'graph_cache/' . $file) + $interval) <
                 time()) && ($file != "..") && ($file != "."))
        {
...
function print_graph ($g, $pgwidth)
{
    ...
    if (isset($g['attr']['LEGEND-OVERLAP']) && $g['attr']['LEGEND-OVERLAP'])
    {
        $overlap = true;
                                                                                                  ...
                                                                                                  // horiz_bar
                                                                                                  // only)
    if (isset($g['attr']['PERCENT']) && $g['attr']['PERCENT'] && $type != 'xy' &&
             $type != 'scatter')
    {
        $percent = true;
    } // Show
    ...
    // Antialias: If true - better quality curves, but graph line will only be
                                                                                                // 1px
                                                                                                // even
                                                                                                // in
                                                                                                // PDF
                                                                                                ...
                                                                                                // and
                                                                                                // radar
    if (isset($g['attr']['ANTIALIAS']) &&
             ($g['attr']['ANTIALIAS'] == '' || $g['attr']['ANTIALIAS'] == 0))
    {
        $antialias = false;
    }
    else 
    ...
    if (isset($g['attr']['DATA-ROW-BEGIN']) &&
             ($g['attr']['DATA-ROW-BEGIN'] === '0' ||
             $g['attr']['DATA-ROW-BEGIN'] > 0))
    {
        $rowbegin = $g['attr']['DATA-ROW-BEGIN'];
    }
    
    if (isset($g['attr']['DATA-COL-BEGIN']) &&
             ($g['attr']['DATA-COL-BEGIN'] === '0' ||
             $g['attr']['DATA-COL-BEGIN'] > 0))
    {
        $colbegin = $g['attr']['DATA-COL-BEGIN'];
    }
    
    if (isset($g['attr']['DATA-ROW-END']) &&
             ($g['attr']['DATA-ROW-END'] === '0' ||
             $g['attr']['DATA-ROW-END'] != 0))
    {
        $rowend = $g['attr']['DATA-ROW-END'];
    }
    if (isset($g['attr']['DATA-COL-END']) &&
             ($g['attr']['DATA-COL-END'] === '0' ||
             $g['attr']['DATA-COL-END'] != 0))
    {
    ...
    if ($dataseries == 'cols')
    {
        if ($colbegin > 1)
        {
            for ($r = ($rowbegin - 1); $r < $rowend; $r ++)
            {
                ...
                $longestlegend = max($longestlegend, 
                        strlen($g['data'][$r][0]));
            }
        }
        if ($rowbegin > 1)
    ...
    if (count($data) > 0 && $type)
    {
        ...
        if ($type == 'pie')
        {
            ...
            if ($show_values)
            {
                ...
                // Enable and set policy for guide-lines. Make labels line up
                // vertically
            ...
            // RADAR
                ...
                if ($type == 'radar')
                {
                    ...
                    foreach ($data as $series => $dat)
                    {
                        ...
                        {
                            die(
                                    "ERROR::Graph::Cannot create a Radar Plot with less than 3 data points.");
                        }
                ...
                // LINE
                    ...
                    {
                        // Setup the graph.
                        $graph->img->SetMargin($pml * $k, $pmr * $k, $pmt * $k, 
                                $pmb * $k); // LRTB
                        $graph->SetScale($axes);
                        ...
                        if ($ylabel)
                        {
                            $graph->yaxis->title->SetFont(FF_USERFONT, 
                                    FS_NORMAL, 8 * $k);
                        ...
                        if ($xlabel)
                        {
                            $graph->xaxis->title->SetFont(FF_USERFONT, 
                                    FS_NORMAL, 8 * $k);
                        ...
                        foreach ($data as $series => $rdata)
                        {
                            ...
                            if ($show_values)
                            {
                                ...
                                $bplot->value->SetFont(FF_USERFONT, FS_NORMAL, 
                                        8 * $k);
                                if ($percent || $show_percent)
                                {
                            ...
                            // Indent the X-scale so the first and last point
                            // doesn't fall on the edges
                            $bplot->SetCenter();
                            $graph->Add($bplot);
                        }
                    }
                    // XY or SCATTER
                        ...
                        {
                            // Setup the graph.
                            $graph->img->SetMargin($pml * $k, $pmr * $k, 
                                    $pmt * $k, $pmb * $k); // LRTB
                            $graph->SetScale($axes);
                            // Setup font for axis
                            ...
                            if ($show_values)
                            {
                                ...
                                $cplot->value->SetFont(FF_USERFONT, FS_NORMAL, 
                                        6 * $k);
                        ...
                        // BAR
                            ...
                            if ($type == 'bar')
                            {
                                ...
                                $graph->SetScale($axes);
                                // Setup y-axis
                                $graph->yaxis->SetFont(FF_USERFONT, FS_NORMAL, 
                                        8 * $k);
                                if ($hide_y_axis)
                                {
                                    $graph->yaxis->Hide();
                                ...
                                // Show 0 label on Y-axis (default is not to
                                // show)
                                $graph->yscale->ticks->SupressZeroLabel(false);
                                // Setup X-axis labels
                                $graph->xaxis->SetFont(FF_USERFONT, FS_NORMAL, 
                                        8 * $k);
                                $graph->xaxis->SetTickLabels($legends);
                                $graph->xaxis->SetLabelAngle($xlangle);
                                $graph->xaxis->SetLabelMargin(4 * $k);
                                ...
                                foreach ($data as $series => $dat)
                                {
                                    ...
                                    {
                                        $bplot->SetFillGradient($fills[$series], 
                                    ...
                                    if ($show_values)
                                    {
                                        ...
                                        $bplot->value->SetMargin(6 * $k);
                                        $bplot->value->SetColor("darkred");
                                        $bplot->value->SetFont(FF_USERFONT, 
                                                FS_NORMAL, 8 * $k);
                                        if ($percent || $show_percent)
                                        {
                                            $bplot->value->SetFormat('%d%%');
                                ...
                                {
                                    $graph->SetScale($axes);
                                    $graph->Set90AndMargin($pml * $k, $pmr * $k, 
                                            $pmt * $k, $pmb * $k); // LRTB
                                                                                                     
                                    // Setup y-axis
                                    $graph->yaxis->SetFont(FF_USERFONT, 
                                            FS_NORMAL, 8 * $k);
                                    $graph->yaxis->SetLabelMargin(4 * $k);
                                    
                                    $graph->yaxis->SetPos('max'); // Intersect
                                                                  // at top of
                                                                  // x-axis i.e.
                                                                  // y axis is
                                                                  // at bottom
                                                                  ...
                                                                  // the labels
                                                                  // look right
                                    $graph->yaxis->SetLabelAlign('center', 
                                            'top');
                                    if ($percent || $show_percent)
                                    {
                                        $graph->yaxis->SetLabelFormat('%d%%');
                                    ...
                                    $graph->yaxis->scale->SetGrace(10); // sets
                                                                        // 10%
                                                                        // headroom
                                    if ($hide_y_axis)
                                    {
                                    ...
                                    $graph->yscale->ticks->SupressZeroLabel(
                                            false);
                                    // Setup X-axis labels
                                    $graph->xaxis->SetFont(FF_USERFONT, 
                                            FS_NORMAL, 8 * $k);
