...
// svg2pdf fpdf class
// sylvain briand (syb@godisaduck.com), modified by rick trevino
// (rtrevino1@yahoo.com)
// http://www.godisaduck.com/svg2pdf_with_fpdf
// http://rhodopsin.blogspot.com
//
// cette class etendue est open source, toute modification devra cependant etre
// repertori�e~

// NB UNITS - Works in pixels as main units - converting to PDF units when
// outputing to PDF string
// and on returning size
class SVG
{

    var $svg_gradient; // array - contient les infos sur les gradient fill du
                       // svg class� par id du svg
    ...
    function SVG (&$mpdf)
    {
        ...
        $this->kp = 72 / $mpdf->img_dpi; // mPDF 4.4.003 constant To convert
                                         // pixels to pts/PDF units
        $this->kf = 1; // mPDF 5.0.039 constant To convert font size if
                       // re-mapped
        ...
        $this->svg_style = array(
                array(
                        ...
                        'fill-rule' => 'nonzero', // mode de remplissage par
                                                   // defaut
                        'stroke' => 'none', // pas de trait par defaut
                        'stroke-linecap' => 'butt', // style de langle par
                                                    // defaut
        ...
        $this->txt_style = array(
                array(
                        ...
                        'font-size' => $mpdf->default_font_size, // ****** this
                                                                  // is pts
                        'font-weight' => 'normal', // normal | bold
                        'font-style' => 'normal', // italic | normal
                        'text-anchor' => 'start', // alignment: start, middle,
                                                  // end
                        /* mPDF 5.0.041 */
                        'fill-opacity' => 1, // remplissage opaque par defaut
                        'fill-rule' => 'nonzero', // mode de remplissage par
    ...
    function svgGradient ($gradient_info, $attribs, $element)
    {
            ...
            if ($element == 'ellipse')
            {
                ...
                if ($element == 'circle')
                {
                    ...
                    if ($element == 'polygon')
                    {
                        ...
                        {
                            // mPDF 5.0.039
                            if (is_array($this->pathBBox) && $this->pathBBox[2] >
                                     0)
                            {
                                $w = $this->pathBBox[2];
                                $h = $this->pathBBox[3];
                            ...
                            {
                                preg_match_all('/([a-z]|[A-Z])([ ,\-.\d]+)*/', 
                                        $attribs['d'], $commands, PREG_SET_ORDER);
                                $maxr = $maxb = 0;
                                $minl = $mint = 999999;
        ...
        if (isset($gradient_info['transform']))
        {
            ...
            if (count($m[0]))
            {
                for ($i = 0; $i < count($m[0]); $i ++)
                {
                    ...
                    {
                        // mPDF 5.0.039
                        // Note angle of rotation is reversed (from SVG to PDF),
                        // so vv[1] and vv[2] are negated
                        // cf svgDefineStyle()
                        $transformations .= sprintf(
                                ' %.3F %.3F %.3F %.3F %.3F %.3F cm ', $vv[0], 
                                - $vv[1], - $vv[2], $vv[3], $vv[4] * $this->kp, 
                                - $vv[5] * $this->kp);
                    }
                    else 
        ...
        // mPDF 5.0.039
        if (isset($gradient_info['units']) &&
                 strtolower($gradient_info['units']) == 'userspaceonuse')
        {
            if ($transformations)
            {
        ...
        for ($i = 0; $i < (count($gradient_info['color'])); $i ++)
        {
            ...
            {
                $gradient_info['color'][$i]['offset'] = ($gradient_info['color'][$i]['offset'] +
                         0) / 100;
            }
            if (stristr($gradient_info['color'][($i + 1)]['offset'], '%') !==
                     false)
            {
                $gradient_info['color'][($i + 1)]['offset'] = ($gradient_info['color'][($i +
                 1)]['offset'] + 0) / 100;
    }
    ...
    if ($i > 0)
    {
        if ($gradient_info['color'][$i]['offset'] <
                 $gradient_info['color'][($i - 1)]['offset'])
        {
            $gradient_info['color'][$i]['offset'] = $gradient_info['color'][($i -
             1)]['offset'];
}
}
}
...
{
    $gradient_info['info']['x1'] = ($gradient_info['info']['x1'] - $x_offset) /
     $w;
}
...
{
$angle = rad2deg(
atan2(($gradient_info['info']['y2'] - $gradient_info['info']['y1']), 
...
if ($angle != 0 && $angle != 360 && $angle != 90 && $angle != 180 && $angle !=
 270)
{
if ($w >= $h)
{
...
$return .= sprintf('%.3F 0 0 %.3F %.3F %.3F cm ', $a * $this->kp, $d * $this->kp, 
$e * $this->kp, $f * $this->kp);

// mPDF 5.0.039
if (isset($gradient_info['units']) &&
 strtolower($gradient_info['units']) == 'objectboundingbox')
{
if ($transformations)
{
$return .= $transformations;
...
if ($spread == 'F' && ($gp % 2) == 1)
{ // Reflect
    $gradient_info['color'][(($ns * $gp) + $i)] = $gradient_info['color'][(($ns *
             ($gp - 1)) + ($ns - $i - 1))];
...
for ($i = ($ns - 1); $i >= 0; $i --)
{
if (isset($newarr[$i]['offset'])) array_unshift($gradient_info['color'], 
        $newarr[$i]);
}
}
}
...
// mPDF 5.0.042
$range = $gradient_info['color'][count($gradient_info['color']) - 1]['offset'] -
 $gradient_info['color'][0]['offset'];
...
if ($gradient_info['colorspace'] == 'Gray') $gradient_info['color'][$i]['color'] = '0';
else 
    if ($gradient_info['colorspace'] == 'CMYK') $gradient_info['color'][$i]['color'] = '1 1 1 1';
}
...
$grx2 = $x1 +
 ($x2 - $x1) *
 $gradient_info['color'][count($gradient_info['color']) - 1]['offset'];
$gry2 = $y1 +
 ($y2 - $y1) *
 $gradient_info['color'][count($gradient_info['color']) - 1]['offset'];

$this->mpdf_ref->gradients[$n]['coords'] = array(
$grx1,
$gry1,
$grx2,
$gry2
...
$this->mpdf_ref->gradients[$n]['colorspace'] = $gradient_info['colorspace']; // mPDF
                                                                             // 5.0.051

$this->mpdf_ref->gradients[$n]['type'] = 2;
$this->mpdf_ref->gradients[$n]['fo'] = true;

$this->mpdf_ref->gradients[$n]['extend'] = array(
'true',
'true'
);
if ($trans)
...
if ($gradient_info['info']['r'] || $gradient_info['info']['r'] === 0)
{
...
{
$angle = rad2deg(
    atan2(($gradient_info['info']['y0'] - $gradient_info['info']['y1']), 
...
// x1 and y1 (fx, fy) should be inside the circle defined by x0 y0 and r else
// error in mPDF
...
$svg_w = $this->mpdf_ref->ConvertSize($attribs['width']); // mm (interprets
                                                          // numbers as pixels)
...
{ // if 'w' set by viewBox
if ($svg_w)
{ // if width also set, use these values to determine to set size of
              // "pixel"
...
{
// Not sure if this is supposed to strip off units, but since I dont use any I
// will omlt this step
...
{
// mPDF 5.0.039
// Note angle of rotation is reversed (from SVG to PDF), so vv[1] and vv[2] are
// negated
$transformations .= sprintf(' %.3F %.3F %.3F %.3F %.3F %.3F cm ', $vv[0], 
        - $vv[1], - $vv[2], $vv[3], $vv[4] * $this->kp, - $vv[5] * $this->kp);
    
    /*
     * // The long way of doing this?? // need to reverse angle of rotation from
     * SVG to PDF $sx=sqrt(pow($vv[0],2)+pow($vv[2],2)); if ($vv[0] < 0) { $sx
     * *= -1; } // change sign $sy=sqrt(pow($vv[1],2)+pow($vv[3],2)); if ($vv[3]
     * < 0) { $sy *= -1; } // change sign // rotation angle is
     * $t=atan2($vv[1],$vv[3]); $t=atan2(-$vv[2],$vv[0]);	// Should be the same
     * value or skew has been applied // Reverse angle $t *= -1; // Rebuild
     * matrix $ma = $sx * cos($t); $mb = $sy * sin($t); $mc = -$sx * sin($t);
     * $md = $sy * cos($t); // $transformations .= sprintf(' %.3F %.3F %.3F %.3F
     * %.3F %.3F cm ', $ma, $mb, $mc, $md, $vv[4]*$this->kp, -$vv[5]*$this->kp);
     */
}
else 
if ($c == 'translate' && count($vv))
{
    ...
    if ($c == 'scale' && count($vv))
    {
        ...
        if ($c == 'rotate' && count($vv))
        {
            ...
            if (count($vv) == 3)
            {
                $transformations .= sprintf(' 1 0 0 1 %.3F %.3F cm ', 
                        $vv[1] * $this->kp, - $vv[2] * $this->kp);
            }
            $transformations .= sprintf(' %.3F %.3F %.3F %.3F 0 0 cm ', $tm[0], 
                    $tm[1], $tm[2], $tm[3]);
            if (count($vv) == 3)
            {
                $transformations .= sprintf(' 1 0 0 1 %.3F %.3F cm ', 
...
{
if (preg_match('/fill:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)/', 
$critere_style['style'], $m))
{
$current_style['fill'] = '#' . str_pad(dechex($m[1]), 2, "0", STR_PAD_LEFT) .
 str_pad(dechex($m[2]), 2, "0", STR_PAD_LEFT) .
 str_pad(dechex($m[3]), 2, "0", STR_PAD_LEFT);
}
else
{
$tmp = preg_replace("/(.*)fill:\s*([a-z0-9#_()]*|none)(.*)/i", "$2", 
$critere_style['style']); // mPDF
                                                                                                    // 4.4.003
...
$tmp = preg_replace("/(.*)fill-opacity:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)fill-rule:\s*([a-z0-9#]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
if (preg_match('/stroke:\s*rgb\((\d+),\s*(\d+),\s*(\d+)\)/', 
$critere_style['style'], $m))
{
$current_style['stroke'] = '#' . str_pad(dechex($m[1]), 2, "0", STR_PAD_LEFT) .
 str_pad(dechex($m[2]), 2, "0", STR_PAD_LEFT) .
 str_pad(dechex($m[3]), 2, "0", STR_PAD_LEFT);
}
else
{
$tmp = preg_replace("/(.*)stroke:\s*([a-z0-9#]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)stroke-linecap:\s*([a-z0-9#]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)stroke-linejoin:\s*([a-z0-9#]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)stroke-miterlimit:\s*([a-z0-9#]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)stroke-opacity:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)stroke-width:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
// mPDF 4.4.003
$tmp = preg_replace("/(.*)stroke-dasharray:\s*([a-z0-9., ]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
// mPDF 4.4.003
$tmp = preg_replace("/(.*)stroke-dashoffset:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
{
$fill_gradient = $this->svgGradient($this->svg_gradient[$id_gradient], $attribs, 
    $element);
if ($fill_gradient)
{ // mPDF 4.4.003
$path_style = "q ";
...
if (strtolower($critere_style['fill']) == 'currentcolor')
{
...
{
    /*
     * // Cannot put a gradient on a "stroke" in PDF? $id_gradient =
     * preg_replace("/url\(#([\w_]*)\)/i","$1",$critere_style['stroke']); if
     * ($id_gradient != $critere_style['stroke']) { if
     * (isset($this->svg_gradient[$id_gradient])) { $fill_gradient =
     * $this->svgGradient($this->svg_gradient[$id_gradient], $attribs,
     * $element); if ($fill_gradient) { $path_style = "q "; $w = "W"; $style .=
     * 'D'; } } }
     */
}
// mPDF 4.4.005 Used as indirect setting for currentColor
...
$path_style .= $this->mpdf_ref->SetDColor($col, true) . ' '; // mPDF 5.0.051
$style .= 'D';
$lw = $this->ConvertSVGSizePixels($critere_style['stroke-width']); // mPDF
                                                                   // 4.4.003
$path_style .= sprintf('%.3F w ', $lw * $this->kp);
}
}
...
$gs = $this->mpdf_ref->AddExtGState(array(
'ca' => $opacity,
'BM' => '/Normal'
));
...
// mPDF 4.4.003
preg_match_all('/[\-^]?[\d.]+(e[\-]?[\d]+){0,1}/i', $arguments, $a, 
PREG_SET_ORDER);

// if the command is a capital letter, the coords go absolute, otherwise
// relative
...
if ($i == 0) $path_cmd .= sprintf('%.3F %.3F m ', $pdf_pt['x'] * $this->kp, 
    $pdf_pt['y'] * $this->kp);
else $path_cmd .= sprintf('%.3F %.3F l ', $pdf_pt['x'] * $this->kp, 
    $pdf_pt['y'] * $this->kp);
...
$mint = min($mint, - $pdf_pt['y']);
$maxb = max($maxb, - $pdf_pt['y']);
$path_cmd .= sprintf('%.3F %.3F l ', $pdf_pt['x'] * $this->kp, 
    $pdf_pt['y'] * $this->kp);
}
break;
...
{
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $pdfx1 * $this->kp, 
        $pdfy1 * $this->kp, $pdfx2 * $this->kp, $pdfy2 * $this->kp, 
        $pdfx * $this->kp, $pdfy * $this->kp);
}
}
break;
...
$this->lastcontrolpoints = array(
    ...
    - ($pdfy - $pdfy2)
                                                                  ...
                                                                  // relative
                                                                  // $pdf_pt2 =
                                                                  // $this->svg_overflow($pdfx2,$pdfy2);
                                                                  // $pdf_pt1 =
                                                                  // $this->svg_overflow($pdfx1,$pdfy1);
$pdf_pt = $this->svg_overflow($pdfx, $pdfy);
...
break;
case 't': // bezier quadratic avec point de control simetrique a lancien point
          // de control
          // mPDF 4.4.003
if (! ($this->lastcommand == 'Q' || $this->lastcommand == 'q' ||
 $this->lastcommand == 'T' || $this->lastcommand == 't'))
{
$this->lastcontrolpoints = array(
    0,
    0
...
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $pdfx1 * $this->kp, 
    $pdfy1 * $this->kp, $pdfx2 * $this->kp, $pdfy2 * $this->kp, $pdfx * $this->kp, 
    $pdfy * $this->kp);
}
...
list ($pcmd, $bounds) = $this->Arcto($x1, $y1, $x2, $y2, $rx, $ry, $angle, 
...
$numerator = $rx * $rx * $ry * $ry - $rx * $rx * $y1dash * $y1dash -
 $ry * $ry * $x1dash * $x1dash;
...
$root = ($largeArcFlag == $sweepFlag ? - 1.0 : 1.0) *
 sqrt($numerator / ($rx * $rx * $y1dash * $y1dash + $ry * $ry * $x1dash *
 $x1dash));
}
...
$theta1 = $this->CalcVectorAngle(1.0, 0.0, ($x1dash - $cxdash) / $rx, 
($y1dash - $cydash) / $ry);
$dtheta = $this->CalcVectorAngle(($x1dash - $cxdash) / $rx, 
($y1dash - $cydash) / $ry, (- $x1dash - $cxdash) / $rx, (- $y1dash - $cydash) /
 $ry);
...
$x2 = $c[4];
$y2 = $c[5];
$path .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $cpx1 * $this->kp, 
- $cpy1 * $this->kp, $cpx2 * $this->kp, - $cpy2 * $this->kp, $x2 * $this->kp, 
- $y2 * $this->kp) . "\n";
...
// e.g. $w =
// $this->ConvertSVGSizePixels($arguments['w'],$this->svg_info['w']*(25.4/$this->mpdf_ref->dpi));
// usefontsize - setfalse for e.g. margins - will ignore fontsize for % values
// Depends of maxsize value to make % work properly. Usually maxsize ==
// pagewidth
...
// Return as pixels
$size = $this->mpdf_ref->ConvertSize($size, $maxsize, $fontsize, false) * 1 /
 (25.4 / $this->mpdf_ref->dpi);
return $size;
}
...
$path_cmd .= sprintf('%.3F %.3F l ', (($x + $w) * $this->kp), 
- (($y + $h) * $this->kp));
...
$path_cmd .= sprintf('%.3F %.3F l ', ($x + ($w - $rx)) * $this->kp, 
- $y * $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', 
($x + ($w - $rx + $kx)) * $this->kp, - $y * $this->kp, ($x + $w) * $this->kp, 
(- $y + (- $ry + $ky)) * $this->kp, ($x + $w) * $this->kp, 
(- $y + (- $ry)) * $this->kp);
$path_cmd .= sprintf('%.3F %.3F l ', ($x + $w) * $this->kp, 
(- $y + (- $h + $ry)) * $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', ($x + $w) * $this->kp, 
(- $y + (- $h - $ky + $ry)) * $this->kp, ($x + ($w - $rx + $kx)) * $this->kp, 
(- $y + (- $h)) * $this->kp, ($x + ($w - $rx)) * $this->kp, 
(- $y + (- $h)) * $this->kp);

$path_cmd .= sprintf('%.3F %.3F l ', ($x + $rx) * $this->kp, 
(- $y + (- $h)) * $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', 
($x + ($rx - $kx)) * $this->kp, (- $y + (- $h)) * $this->kp, $x * $this->kp, 
(- $y + (- $h - $ky + $ry)) * $this->kp, $x * $this->kp, 
(- $y + (- $h + $ry)) * $this->kp);
$path_cmd .= sprintf('%.3F %.3F l ', $x * $this->kp, (- $y + (- $ry)) *
 $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c h ', $x * $this->kp, 
(- $y + (- $ry + $ky)) * $this->kp, ($x + ($rx - $kx)) * $this->kp, 
- $y * $this->kp, ($x + $rx) * $this->kp, - $y * $this->kp);
}
return $path_cmd;
}
...
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', 
($x1 + ($rx * $kappa)) * $this->kp, $y1 * $this->kp, $x2 * $this->kp, 
($y2 + ($ry * $kappa)) * $this->kp, $x2 * $this->kp, $y2 * $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $x2 * $this->kp, 
($y2 - ($ry * $kappa)) * $this->kp, ($x3 + ($rx * $kappa)) * $this->kp, 
$y3 * $this->kp, $x3 * $this->kp, $y3 * $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', 
($x3 - ($rx * $kappa)) * $this->kp, $y3 * $this->kp, $x4 * $this->kp, 
($y4 - ($ry * $kappa)) * $this->kp, $x4 * $this->kp, $y4 * $this->kp);
$path_cmd .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $x4 * $this->kp, 
($y4 + ($ry * $kappa)) * $this->kp, ($x1 - ($rx * $kappa)) * $this->kp, 
$y1 * $this->kp, $x1 * $this->kp, $y1 * $this->kp);
$path_cmd .= 'h ';
...
// mPDF 5.0
$current_style['font-family'] = $this->mpdf_ref->SetFont(
$current_style['font-family'], $style, $size, false);
...
$strokestr = '';
if (isset($current_style['stroke-width']) && $current_style['stroke-width'] > 0 &&
 $current_style['stroke'] != 'none')
{
...
{
$tw = $this->mpdf_ref->GetStringWidth($txt) * $this->mpdf_ref->k / 2; // mPDF
                                                                  // 4.4.003
}
else 
if ($current_style['text-anchor'] == 'end')
{
...
// mPDF 5.0.041
// mPDF 5.0.051
$path_cmd = sprintf('q BT /F%d %s %.3F Tf %.3F %.3F Td %s Tr %s %s %s Tj ET Q ', 
$this->mpdf_ref->CurrentFont['i'], $opacitystr, $this->mpdf_ref->FontSizePt, 
$pdfx * $this->kp, $pdfy * $this->kp, $render, $fillstr, $strokestr, $txt) . "\n";
...
// mPDF 5.0.039
$tmp = preg_replace("/(.*)font-family:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)font-size:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)font-weight:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
$tmp = preg_replace("/(.*)font-style:\s*([a-z0-9.]*|none)(.*)/i", "$2", 
$critere_style['style']);
if ($tmp != $critere_style['style'])
{
...
{
// mPDF 4.4.003
$current_style['font-size'] = $this->ConvertSVGSizePts(
$critere_style['font-size']);
$this->mpdf_ref->SetFont('', '', $current_style['font-size'], false);
}
...
if ((! $this->mpdf_ref->usingCoreFont &&
 in_array($v, $this->mpdf_ref->available_unifonts)) ||
 ($this->mpdf_ref->usingCoreFont &&
 in_array($v, array(
        'courier',
        'times',
        'helvetica',
...
if (in_array($v, $this->mpdf_ref->sans_fonts) ||
     in_array($v, $this->mpdf_ref->serif_fonts) ||
     in_array($v, $this->mpdf_ref->mono_fonts))
{
$current_style['font-family'] = $v;
break;
}
...
// add current style to text style array (will remove it later after writing
// text to svg_string)
...
{
$data = preg_replace('/&' . preg_quote($ent[1][$i], '/') . ';/is', $ent[2][$i], 
$data);
}
}
...
// Get links
preg_match_all(
'/(<(linearGradient|radialgradient)[^>]*)xlink:href=["\']#(.*?)["\'](.*?)\/>/si', 
$data, $links);
if (count($links[0]))
{
$links[5] = array();
} // mPDF 4.5.010
                                               // Delete links from data -
                                               // keeping in $links
for ($i = 0; $i < count($links[0]); $i ++)
{
$links[5][$i] = 'tmpLink' . RAND(100000, 9999999); // mPDF 4.5.010
$data = preg_replace('/' . preg_quote($links[0][$i], '/') . '/is', 
'<MYLINKS' . $links[5][$i] . '>', $data); // mPDF
                                                                                                     // 4.5.010
}
// Get targets
preg_match_all(
'/<(linearGradient|radialgradient)([^>]*)id=["\'](.*?)["\'](.*?)>(.*?)<\/(linearGradient|radialgradient)>/si', 
$data, $m);
$targets = array();
$stops = array();
// keeping in $targets
...
for ($i = 0; $i < count($links[0]); $i ++)
{
$def = $links[1][$i] . ' ' . $links[4][$i] . '>' . $stops[$links[3][$i]] . '</' .
 $links[2][$i] . '>'; // mPDF
                                                                                             // 4.5.010
$data = preg_replace('/<MYLINKS' . $links[5][$i] . '>/is', $def, $data); // mPDF
                                                                      // 4.5.010
}
}
...
$path = $attribs['d'];
// mPDF 4.4.003
preg_match_all('/([MZLHVCSQTAmzlhvcsqta])([e ,\-.\d]+)*/', $path, $commands, 
        PREG_SET_ORDER);
$path_cmd = '';
$svg_class->subPathInit = true; // mPDF 4.4.003
                                // mPDF 5.0.039
$svg_class->pathBBox = array(
        999999,
        999999,
        - 999999,
        - 999999
...
$critere_style = $attribs;
unset($critere_style['x'], $critere_style['y'], $critere_style['rx'], 
        $critere_style['ry'], $critere_style['height'], $critere_style['width']);
$path_style = $svg_class->svgDefineStyle($critere_style);
break;
...
$path_cmd = $svg_class->svgEllipse($arguments);
$critere_style = $attribs;
unset($critere_style['cx'], $critere_style['cy'], $critere_style['rx'], 
        $critere_style['ry']);
$path_style = $svg_class->svgDefineStyle($critere_style);
break;
...
$critere_style = $attribs;
unset($critere_style['x1'], $critere_style['y1'], $critere_style['x2'], 
        $critere_style['y2']);
$path_style = $svg_class->svgDefineStyle($critere_style);
break;
...
if (isset($attribs['style']) and
         preg_match('/stop-color:\s*([^;]*)/i', $attribs['style'], $m))
{
    $color = trim($m[1]);
}
else 
...
// mPDF 4.4.003
if (isset($attribs['style']) and
         preg_match('/stop-opacity:\s*([0-9.]*)/i', $attribs['style'], $m))
{
    $stop_opacity = $m[1];
}
else 
...
if (isset($attribs['xlink:href']))
{
    unset($attribs['xlink:href']); // this should be a hyperlink
                                       // not handled like a xlink:href in other
                                   // elements
...
{ // mPDF 4.4.003
                                     // mPDF 5.0
list ($prestyle, $poststyle) = $svg_class->svgStyle($path_style, $attribs, 
    strtolower($name));
if ($path_style['transformations'])
{ // transformation on an element
$svg_class->svgWriteString(
        " q " . $path_style['transformations'] .
                 " $prestyle $path_cmd $poststyle" . " Q\n");
}
else
{
...
xml_set_element_handler($svg2pdf_xml_parser, "xml_svg2pdf_start", 
"xml_svg2pdf_end");
...
if ($t > 0 && $t < 1)
{
$bounds[$i][] = (pow((1 - $t), 3) * $P0[$i] + 3 * pow((1 - $t), 2) * $t * $P1[$i] +
 3 * (1 - $t) * pow($t, 2) * $P2[$i] + pow($t, 3) * $P3[$i]);
}
continue;
}
$b2ac = pow($b, 2) - 4 * $c * $a;
...
if ($t1 > 0 && $t1 < 1)
{
$bounds[$i][] = (pow((1 - $t1), 3) * $P0[$i] + 3 * pow((1 - $t1), 2) * $t1 *
 $P1[$i] + 3 * (1 - $t1) * pow($t1, 2) * $P2[$i] + pow($t1, 3) * $P3[$i]);
}
$t2 = (- $b - sqrt($b2ac)) / (2 * $a);
if ($t2 > 0 && $t2 < 1)
{
$bounds[$i][] = (pow((1 - $t2), 3) * $P0[$i] + 3 * pow((1 - $t2), 2) * $t2 *
 $P1[$i] + 3 * (1 - $t2) * pow($t2, 2) * $P2[$i] + pow($t2, 3) * $P3[$i]);
}
}
$x = min($bounds[0]);
$x2 = max($bounds[0]);
