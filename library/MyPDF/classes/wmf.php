        ...
        while ($p < strlen($data) && ! $endRecord)
        {
            ...
            switch ($func)
            {
                case 0x020b: // SetWindowOrg
                    ...
                    if (! $wmfdata) $wo = array_reverse(unpack('s2', $parms));
                    break;
                case 0x020c: // SetWindowExt
                             // do not allow window extent to be changed
                             // after drawing has begun
                    if (! $wmfdata) $we = array_reverse(unpack('s2', $parms));
                    break;
                ...
                case 0x012d: // SelectObject
                    ...
                    switch ($obj['type'])
                    {
                        case 'B':
                            ...
                            {
                                $wmfdata .= $this->mpdf->SetFColor(
                                        $this->mpdf->ConvertColor(
                                                'rgb(' . $obj['r'] . ',' .
                                                         $obj['g'] . ',' .
                                                         $obj['b'] . ')'), true) .
                                 "\n";
