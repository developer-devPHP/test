...
class bmp
{
    ...
    function _getBMPimage ($data, $file)
    {
        ...
        if ($this->mpdf->restrictColorSpace == 1 || $this->mpdf->PDFX ||
                 $this->mpdf->restrictColorSpace == 3)
        {
            if (($this->mpdf->PDFA && ! $this->mpdf->PDFAauto) ||
             ($this->mpdf->PDFX && ! $this->mpdf->PDFXauto))
    {
        $this->mpdf->PDFAXwarnings[] = "Image cannot be converted to suitable colour space for PDFA or PDFX file - " .
         $file . " - (Image replaced by 'no-image'.)";
}
return array(
    'error' => "BMP Image cannot be converted to suitable colour space - " .
             $file . " - (Image replaced by 'no-image'.)"
);
}

$biXPelsPerMeter = $this->_fourbytes2int_le(substr($data, 38, 4)); // horizontal
                                                               // pixels per
                                                               // meter, usually
                                                               // set to zero
                                                               // $biYPelsPerMeter=$this->_fourbytes2int_le(substr($data,42,4));
                                                               // // vertical
                                                               // pixels per
                                                               // meter, usually
                                                               // set to zero
...
default:
return array(
        'error' => 'Error parsing BMP image - Unsupported image biBitCount'
);
}
...
case 0: // ESCAPE
    ...
    switch (ord($str[$i]))
    {
        case 0: // NEW LINE
            ...
            if ($padCnt < $lineWidth) $out .= str_repeat(chr(0), $padCnt); // pad
                                                                         // line
            break;
        case 1: // END OF FILE
            $padCnt = $lineWidth - strlen($out) % $lineWidth;
