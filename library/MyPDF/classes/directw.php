...
class directw
{
            ...
            while ($i < $nb)
            {
                ...
                if ($c == "\n")
                {
                    ...
                    if ($nl == 1)
                    {
                        ...
                        $w = $this->mpdf->w - $this->mpdf->rMargin -
                                 $this->mpdf->x;
                        $wmax = ($w -
                                 ($this->mpdf->cMarginL + $this->mpdf->cMarginR));
                    }
                    $nl ++;
                    continue;
                }
                ...
                if ($l > $wmax)
                {
                    ...
                    if ($sep == - 1)
                    {
                        ...
                        $this->mpdf->magic_reverse_dir($tmp, true, 
                                $directionality); // *RTL*
                        $this->mpdf->Cell($w, $h, $tmp, 0, 2, $align, $fill, 
                                $link);
                    }
                    else
                    {
                        ...
                        if ($align == 'J')
                        {
                            ...
                            // 'spaced' properly
                            $tmp = str_replace(chr(194) . chr(160), chr(32), 
                                    $tmp);
                            $len_ligne = $this->mpdf->GetStringWidth($tmp);
                            $nb_carac = mb_strlen($tmp, $this->mpdf->mb_enc);
                            $nb_spaces = mb_substr_count($tmp, ' ', 
                                    $this->mpdf->mb_enc);
                            $inclCursive = false;
                            if ($checkCursive)
                            {
                                if (preg_match(
                                        "/([" . $this->mpdf->pregRTLchars .
                                                 "])/u", $tmp))
                                {
                                    $inclCursive = true;
                                } // *RTL*
                                if (preg_match(
                                        "/([" . $this->mpdf->pregHIchars .
                                                 $this->mpdf->pregBNchars .
                                                 $this->mpdf->pregPAchars .
                                                 "])/u", $tmp))
                                {
                                    $inclCursive = true;
                                } // *INDIC*
                            }
                            list ($charspacing, $ws) = $this->mpdf->GetJspacing(
                                    $nb_carac, $nb_spaces, 
                                    ((($w - 2) - $len_ligne) * _MPDFK), 
                                    $inclCursive);
        ...
        else
        {
            while ($i < $nb)
            {
                ...
                if ($c == "\n")
                {
                    ...
                    // WORD SPACING
                    $this->mpdf->ResetSpacing();
                    $this->mpdf->Cell($w, $h, substr($s, $j, $i - $j), 0, 2, 
                            $align, $fill, $link);
                    $i ++;
                    $sep = - 1;
                    $j = $i;
    ...
    function CircularText ($x, $y, $r, $text, $align = 'top', $fontfamily = '', 
            $fontsizePt = 0, $fontstyle = '', $kerning = 120, $fontwidth = 100)
    {
        if ($font || $fontstyle || $fontsizePt) $this->mpdf->SetFont(
                $fontfamily, $fontstyle, $fontsizePt);
        $kerning /= 100;
        $fontwidth /= 100;
        if ($kerning == 0) $this->mpdf->Error(
                'Please use values unequal to zero for kerning (CircularText)');
        if ($fontwidth == 0) $this->mpdf->Error(
                'Please use values unequal to zero for font width (CircularText)');
        $t = 0;
        ...
        else
        {
            ...
            for ($i = 0; $i < $nb; $i ++)
            {
                ...
                {
                    if (isset(
                            $this->mpdf->CurrentFont['kerninfo'][$lastchar][$char]))
                    {
                        $tk = $this->mpdf->CurrentFont['kerninfo'][$lastchar][$char] *
                                 ($this->mpdf->FontSize / 1000) * $kerning *
                                 $fontwidth;
                        $w[$i] += $tk / 2;
                        $w[$i - 1] += $tk / 2;
                        $t += $tk;
        ...
        for ($i = 0; $i < $nb; $i ++)
        {
            if ($align == 'top')
            {
                ...
                if ($i == 0)
                {
                    $this->mpdf->transformRotate((($w[$i] / 2) / $u) * 360, $x, 
                            $y);
                }
                else
                {
                    $this->mpdf->transformRotate((($w[$i] / 2 + $w[$i - 1] / 2) /
                             $u) * 360, $x, $y);
                }
                if ($fontwidth != 1)
                {
            ...
            else
            {
                ...
                if ($i == 0)
                {
                    $this->mpdf->transformRotate(- (($w[$i] / 2) / $u) * 360, 
                            $x, $y);
                }
                else
                {
                    $this->mpdf->transformRotate(
                            - (($w[$i] / 2 + $w[$i - 1] / 2) / $u) * 360, $x, $y);
                }
                if ($fontwidth != 1)
                {
                    $this->mpdf->StartTransform();
                ...
                $this->mpdf->SetXY($x - $w[$i] / 2, 
                        $y + $r - ($this->mpdf->FontSize));
            }
            if ($this->mpdf->usingCoreFont)
            {
                $c = $text[$i];
    ...
    function Shaded_box ($text, $font = '', $fontstyle = 'B', $szfont = '', 
            $width = '70%', $style = 'DF', $radius = 2.5, $fill = '#FFFFFF', $color = '#000000', $pad = 2)
    {
