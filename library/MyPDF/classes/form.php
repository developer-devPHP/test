...
class form
{
    ...
    function form (&$mpdf)
    {
        ...
        $this->formSubmitNoValueFields = true; // Whether to include blank
                                               // fields when submitting data
        $this->formSelectDefaultOption = true; // for Select drop down box; if
                                               // no option is explicitly maked
                                               // as selected,
                                               // this determines whether to
                                               // select 1st option (as per
                                               // browser)
                                               // - affects whether "required"
                                               // attribute is relevant
        $this->formUseZapD = true; // Determine whether to use ZapfDingbat icons
                                   // for radio/checkboxes
                                   
        // FORM STYLES
                                   // These can alternatively use a 4 number
                                   // string to represent CMYK colours
        ...
        $this->form_element_spacing['select']['outer']['h'] = 0.5; // Horizontal
                                                                   // spacing
                                                                   // around
                                                                   // SELECT
        $this->form_element_spacing['select']['outer']['v'] = 0.5; // Vertical
                                                                   // spacing
                                                                   // around
                                                                   // SELECT
        $this->form_element_spacing['select']['inner']['h'] = 0.7; // Horizontal
                                                                   // padding
                                                                   // around
                                                                   // SELECT
        $this->form_element_spacing['select']['inner']['v'] = 0.7; // Vertical
                                                                   // padding
                                                                   // around
                                                                   // SELECT
    ...
    function print_ob_text ($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        {
            // Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea;
            // 14 - Password
            $flags = array();
            ...
            $this->SetFormText($w, $h, $objattr['fieldname'], $val, $val, 
                    $objattr['title'], $flags, $fieldalign, false, 
                    $objattr['maxlength'], $js, $objattr['background-col'], 
                    $objattr['border-col']);
        ...
        else
        {
            ...
            $this->mpdf->x += $this->form_element_spacing['input']['outer']['h'] /
                     $k;
            $this->mpdf->y += $this->form_element_spacing['input']['outer']['v'] /
                     $k;
            // Chop texto to max length $w-inner-padding
            while ($this->mpdf->GetStringWidth($texto) >
                     $w - ($this->form_element_spacing['input']['inner']['h'] * 2))
                    {
                        $texto = mb_substr($texto, 0, 
                                mb_strlen($texto, $this->mpdf->mb_enc) - 1, 
                                $this->mpdf->mb_enc);
            }
            ...
            $this->mpdf->Cell($w, $h, $texto, 1, 0, $rtlalign, 1, '', 0, 
                    ...
                    $this->form_element_spacing['input']['inner']['h'] / $k, 'M');
    ...
    function print_ob_textarea ($objattr, $w, $h, $texto, $rtlalign, $k, 
            $blockdir)
    {
        // TEXTAREA
        if ($this->mpdf->useActiveForms)
        {
            ...
            $this->SetFormText($w, $h, $objattr['fieldname'], $texto, $texto, 
                    $objattr['title'], $flags, $fieldalign, false, - 1, $js, 
                    $objattr['background-col'], $objattr['border-col']);
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        }
        else
        {
            ...
            $this->mpdf->x += $this->form_element_spacing['textarea']['outer']['h'] /
                     $k;
            $this->mpdf->y += $this->form_element_spacing['textarea']['outer']['v'] /
                     $k;
            $this->mpdf->SetLineWidth(0.2 / $k);
            if (isset($objattr['disabled']) && $objattr['disabled'])
            {
            ...
            $this->mpdf->x += $this->form_element_spacing['textarea']['inner']['h'] /
                     $k;
            $this->mpdf->y += $this->form_element_spacing['textarea']['inner']['v'] /
                     $k;
            $linesneeded = $this->mpdf->WordWrap($texto, $w);
            if ($linesneeded > $objattr['rows'])
            { // Too many words inside
                ...
                $texto = mb_substr($texto, 0, 
                        mb_strlen($texto, $this->mpdf->mb_enc) - 4, 
                        $this->mpdf->mb_enc) . "...";
            }
            if ($texto != '') $this->mpdf->MultiCell($w, 
                    $this->mpdf->FontSize * $this->textarea_lineheight, $texto, 
                    0, '', 0, '', $blockdir, true);
            $this->mpdf->SetFColor($this->mpdf->ConvertColor(255));
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        }
    ...
    function print_ob_select ($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms)
        {
            // Flags: 1 - Readonly; 2 - Required; 3 - No export; 19 - edit (only
            // if combo)
            $flags = array();
            if ($objattr['disabled'])
            {
            ...
            if (! $objattr['spellcheck'] || $objattr['size'] > 1 ||
                     ! $objattr['editable'])
            {
                $flags[] = 23;
            } // DoNotSpellCheck
            ...
            $this->mpdf->SetTColor($objattr['color']);
            $this->SetFormChoice($w, $h, $objattr['fieldname'], $flags, $data, 
                    $rtlalign, $js);
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        }
        else
        {
            ...
            $this->mpdf->x += $this->form_element_spacing['select']['outer']['h'] /
                     $k;
            $this->mpdf->y += $this->form_element_spacing['select']['outer']['v'] /
                     $k;
            $this->mpdf->Cell($w - ($this->mpdf->FontSize * 1.4), $h, $texto, 1, 
                    0, $rtlalign, 1, '', 0, 
            ...
            if ($this->mpdf->PDFA || $this->mpdf->PDFX)
            {
                if (($this->mpdf->PDFA && ! $this->mpdf->PDFAauto) ||
                         ($this->mpdf->PDFX && ! $this->mpdf->PDFXauto))
                {
                ...
                if ($this->mpdf->_charDefined($this->mpdf->CurrentFont['cw'], 
                        9660))
                {
                    $down = "\xe2\x96\xbc";
                }
                ...
                $this->mpdf->Cell(($this->mpdf->FontSize * 1.4), $h, $down, 1, 
                        0, 'C', 1, '', 0, 0, 0, 'M');
            }
            else
            {
                $this->mpdf->SetFont('czapfdingbats', '', 0);
                $this->mpdf->Cell(($this->mpdf->FontSize * 1.4), $h, chr(116), 
                        1, 0, 'C', 1, '', 0, 0, 0, 'M');
            }
    ...
    function print_ob_imageinput ($objattr, $w, $h, $texto, $rtlalign, $k, 
            $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms)
        {
            ...
            $this->SetJSButton($w, $h, $objattr['fieldname'], $objattr['value'], 
                    $js, $objattr['ID'], $objattr['title'], $flags, 
                    $objattr['Indexed']);
        }
        else
        {
            ...
            $this->mpdf->_out(
                    sprintf("q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q", 
                            $objattr['INNER-WIDTH'] * _MPDFK, 
                            $objattr['INNER-HEIGHT'] * _MPDFK, 
    ...
    function print_ob_button ($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms)
        {
            ...
            if ($objattr['subtype'] == 'RESET')
            {
                ...
                $this->SetFormReset($w, $h, $objattr['fieldname'], 
                        $objattr['value'], $objattr['title'], $flags, 
                        $objattr['background-col'], $objattr['border-col'], 
                        $objattr['noprint']);
            }
            else 
                if ($objattr['subtype'] == 'SUBMIT')
                {
                    $url = $this->formAction;
                    ...
                    $this->SetFormSubmit($w, $h, $objattr['fieldname'], 
                            $objattr['value'], $url, $objattr['title'], $type, 
                            $method, $flags, $objattr['background-col'], 
                            $objattr['border-col'], $objattr['noprint']);
                }
                else 
                    if ($objattr['subtype'] == 'BUTTON')
                    {
                        $this->SetFormButtonText($objattr['value']);
                        if ($objattr['onClick'])
                        {
                        ...
                        $this->SetJSButton($w, $h, $objattr['fieldname'], 
                                $objattr['value'], $js, 0, $objattr['title'], 
                                $flags, false, $objattr['background-col'], 
                                $objattr['border-col'], $objattr['noprint']);
                    }
            $this->mpdf->SetTColor($this->mpdf->ConvertColor(0));
        }
        else
        {
            ...
            $this->mpdf->x += $this->form_element_spacing['button']['outer']['h'] /
                     $k;
            $this->mpdf->y += $this->form_element_spacing['button']['outer']['v'] /
                     $k;
            $this->mpdf->RoundedRect($this->mpdf->x, $this->mpdf->y, $w, $h, 
                    0.5 / $k, 'DF');
            $w -= $this->form_element_spacing['button']['inner']['h'] * 2 / $k;
            $h -= $this->form_element_spacing['button']['inner']['v'] * 2 / $k;
            $this->mpdf->x += $this->form_element_spacing['button']['inner']['h'] /
                     $k;
            $this->mpdf->y += $this->form_element_spacing['button']['inner']['v'] /
                     $k;
            $save_linemaxfontsize = $this->mpdf->linemaxfontsize;
            $this->mpdf->linemaxfontsize = $this->mpdf->FontSize;
            $this->mpdf->magic_reverse_dir($texto, false, $blockdir); // *RTL*
    ...
    function print_ob_checkbox ($objattr, $w, $h, $texto, $rtlalign, $k, 
            $blockdir, $x, $y)
    {
        // CHECKBOX
        if ($this->mpdf->useActiveForms)
        {
            ...
            $this->SetCheckBox($w, $h, $objattr['fieldname'], $objattr['value'], 
                    $objattr['title'], $checked, $flags, $objattr['disabled']);
            if ($this->formUseZapD)
            {
                $this->mpdf->SetFont($save_font, '', 0);
                $this->mpdf->currentfontfamily = $save_currentfont;
    ...
    function print_ob_radio ($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, 
            $x, $y)
    {
        // RADIO
        if ($this->mpdf->useActiveForms)
    ...
    function _putRadioItems ($n)
    {
        ...
        foreach ($this->form_radio_groups as $name => $frg)
        {
            ...
            $this->mpdf->_out(
                    '/NM ' .
                             $this->mpdf->_textstring(
                                    sprintf('%04u-%04u', $n, (3000 + $key ++))));
            $this->mpdf->_out(
                    '/M ' . $this->mpdf->_textstring('D:' . date('YmdHis')));
            ...
            // V entry holds index corresponding to the appearance state of
                                                       // whichever child field
                                                       // is currently in the on
                                                       // state = or Off
            if (isset($frg['on']))
            {
                $state = $frg['on'];
    ...
    function _putFormsCatalog ()
    {
        if (isset($this->pdf_acro_array))
        {
            ...
            foreach ($this->form_fonts as $fn)
            {
                ...
                {
                    $this->mpdf->Error(
                            "Cannot use fonts with SMP or SIP characters for interactive Form elements");
                }
                $f .= '/F' . $this->mpdf->fonts[$fn]['i'] . ' ' .
                         $this->mpdf->fonts[$fn]['n'] . ' 0 R ';
            }
    ...
    function SetFormTextJS ($name, $js)
    {
        for ($i = 0; $i < count($js); $i ++)
        {
            ...
            if ($name)
            {
                $this->array_form_text_js[$this->mpdf->_escape($name)][$format] = array(
                        'js' => $j
                );
            }
        }
    ...
    function SetFormText ($w, $h, $name, $value = '', $default = '', $title = '', 
            $flags = array(), $align = 'L', $hidden = false, $maxlen = -1, $js = '', 
            $background_col = false, $border_col = false)
    {
        // Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea; 14 -
        // Password
        $this->formn ++;
        if ($align == 'C')
        {
        ...
        if (! preg_match('/^[a-zA-Z0-9_:\-]+$/', $name))
        {
            $this->mpdf->Error(
                    "Field [" . $name .
                             "] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)");
        }
        ...
        else
        {
            ...
            {
                $this->mpdf->UTF8StringToArray($value, true); // Add characters
                                                              // to font subset
                $this->mpdf->UTF8StringToArray($default, true); // Add
                                                                // characters to
                                                                // font subset
                $this->mpdf->UTF8StringToArray($title, true); // Add characters
                                                              // to font subset
            }
        ...
        if ($this->mpdf->keep_block_together)
        {
            ...
            else
            {
                if ($this->mpdf->ColActive)
                {
                    ...
                    $this->mpdf->columnForms[$this->mpdf->CurrCol][INTVAL(
                            $this->mpdf->x)][INTVAL($this->mpdf->y)] = $this->formn;
                }
    ...
    function SetFormChoice ($w, $h, $name, $flags, $array, $align = 'L', $js = '')
    {
        ...
        if ($this->mpdf->onlyCoreFonts)
        {
            ...
            {
                $array['VAL'][$i] = $this->Win1252ToPDFDocEncoding(
                        $array['VAL'][$i]);
                $array['OPT'][$i] = $this->Win1252ToPDFDocEncoding(
                        $array['OPT'][$i]);
            }
        }
        else
        {
            for ($i = 0; $i < count($array['VAL']); $i ++)
            {
                if (isset($this->mpdf->CurrentFont['subset']))
                {
                    $this->mpdf->UTF8StringToArray($array['VAL'][$i], true); // Add
                                                                             // characters
                                                                             // to
                                                                             // font
                                                                             // subset
                    $this->mpdf->UTF8StringToArray($array['OPT'][$i], true); // Add
                                                                             // characters
                                                                             // to
                                                                             // font
                                                                             // subset
                }
                if ($array['VAL'][$i]) $array['VAL'][$i] = $this->mpdf->UTF8ToUTF16BE(
                        $array['VAL'][$i], true);
                if ($array['OPT'][$i]) $array['OPT'][$i] = $this->mpdf->UTF8ToUTF16BE(
                        $array['OPT'][$i], true);
            }
        }
        $f = array(
    ...
    // CHECKBOX
    function SetCheckBox ($w, $h, $name, $value, $title = '', $checked = false, 
            $flags = array(), $disabled = false)
    {
        $this->SetFormButton($w, $h, $name, $value, 'checkbox', $title, $flags, 
                $checked, $disabled);
        $this->mpdf->x += $w;
    }
    
    // RADIO
    function SetRadio ($w, $h, $name, $value, $title = '', $checked = false, 
            $flags = array(), $disabled = false)
    {
        $this->SetFormButton($w, $h, $name, $value, 'radio', $title, $flags, 
                $checked, $disabled);
        $this->mpdf->x += $w;
    }

    function SetFormReset ($w, $h, $name, $value = 'Reset', $title = '', $flags = array(), 
            $background_col = false, $border_col = false, $noprint = false)
    {
        if (! $name)
        {
            $name = 'Reset';
        }
        $this->SetFormButton($w, $h, $name, $value, 'reset', $title, $flags, 
                false, false, $background_col, $border_col, $noprint);
        $this->mpdf->x += $w;
    }

    function SetJSButton ($w, $h, $name, $value, $js, $image_id = 0, $title = '', 
            $flags = array(), $indexed = false, $background_col = false, $border_col = false, 
            $noprint = false)
    {
        $this->SetFormButton($w, $h, $name, $value, 'js_button', $title, $flags, 
                false, false, $background_col, $border_col, $noprint);
    ...
    function SetFormSubmit ($w, $h, $name, $value = 'Submit', $url, $title = '', 
            $typ = 'html', $method = 'POST', $flags = array(), $background_col = false, 
            $border_col = false, $noprint = false)
    {
        if (! $name)
        {
            $name = 'Submit';
        }
        $this->SetFormButton($w, $h, $name, $value, 'submit', $title, $flags, 
                false, false, $background_col, $border_col, $noprint);
    ...
    function SetFormButtonText ($ca, $rc = '', $ac = '')
    {
        ...
        else
        {
            if (isset($this->mpdf->CurrentFont['subset']))
            {
                $this->mpdf->UTF8StringToArray($ca, true); // Add characters to
                                                           // font subset
            }
    ...
    function SetFormButton ($bb, $hh, $name, $value, $type, $title = '', 
            $flags = array(), $checked = false, $disabled = false, $background_col = false, 
            $border_col = false, $noprint = false)
    {
        $this->formn ++;
        if (! preg_match('/^[a-zA-Z0-9_:\-]+$/', $name))
        ...
        {
            $title = $this->Win1252ToPDFDocEncoding($title);
            $value = $this->Win1252ToPDFDocEncoding($value); // // ??? not
                                                             // needed
        ...
        if ($type == 'radio' || $type == 'checkbox')
        {
            if (! preg_match('/^[a-zA-Z0-9_:\-\.]+$/', $value))
            {
                $this->mpdf->Error(
                        "Field '" . $name .
                                 "' must have a value, which can only contain letters, numbers, colon(:), undersore(_), hyphen(-) or period(.)");
            }
        }
        if ($type == 'radio')
        {
            ...
            // Disable the whole radio group if one is disabled, because of
            // inconsistency in PDF readers
            if ($disabled)
            {
    ...
    {
        if (($r == 0 and $g == 0 and $b == 0) || $g == - 1) $this->form_border_color = sprintf(
                '%.3F', $r / 255);
        else $this->form_border_color = sprintf('%.3F %.3F %.3F', $r / 255, 
                $g / 255, $b / 255);
    }

    function SetFormBackgroundColor ($r, $g = -1, $b = -1)
    {
        if (($r == 0 and $g == 0 and $b == 0) || $g == - 1) $this->form_background_color = sprintf(
                '%.3F', $r / 255);
        else $this->form_background_color = sprintf('%.3F %.3F %.3F', $r / 255, 
                $g / 255, $b / 255);
    }
    ...
    function _put_button_icon ($array, $w, $h)
    {
        ...
        else
        {
            ...
            if ($info['cs'] == 'DeviceCMYK') if ($info['type'] == 'jpg')
            {
                $this->mpdf->_out('/Decode [1 0 1 0 1 0 1 0]');
            }
        }
        if (isset($info['f'])) $this->mpdf->_out('/Filter /' . $info['f']);
        if (isset($info['parms'])) $this->mpdf->_out($info['parms']);
    ...
    function _putform_bt ($form, $hPt)
    {
        ...
        $this->mpdf->_newobj();
        $n = $this->mpdf->n;
        if ($form['subtype'] != 'radio') $this->pdf_acro_array .= $n . ' 0 R '; // Add
                                                                              // to
                                                                              // /Field
                                                                              // element
        ...
        $this->mpdf->_out(
                '/NM ' .
                         $this->mpdf->_textstring(
                                sprintf('%04u-%04u', $n, (7000 + $form['n']))));
        $this->mpdf->_out('/M ' . $this->mpdf->_textstring('D:' .
                 date('YmdHis')));
        $this->mpdf->_out(
                '/Rect [ ' .
                         $this->_form_rect($form['x'], $form['y'], $form['w'], 
                                $form['h'], $hPt) . ' ]');
        $form['noprint'] ? $this->mpdf->_out('/F 0 ') : $this->mpdf->_out(
                '/F 4 ');
        $this->mpdf->_out('/FT /Btn ');
        $this->mpdf->_out('/H /P ');
        ...
        if ($form['subtype'] == 'checkbox')
        {
            ...
            {
                $this->mpdf->_out(
                        '/DA (/F' . $this->mpdf->fonts['czapfdingbats']['i'] .
                                 ' 0 Tf ' . $radio_color . ' rg)');
                $this->mpdf->_out(
                        "/AP << /N << /" . $this->mpdf->_escape($form['V']) . " " .
                                 ($this->mpdf->n + 1) . " 0 R /Off /Off >> >>");
            }
            else
            {
                $this->mpdf->_out(
                        '/DA (/F' .
                                 $this->mpdf->fonts[$this->mpdf->CurrentFont['fontkey']]['i'] .
                                 ' 0 Tf ' . $radio_color . ' rg)');
                $this->mpdf->_out(
                        "/AP << /N << /" . $this->mpdf->_escape($form['V']) . " " .
                                 ($this->mpdf->n + 1) . " 0 R /Off " .
                                 ($this->mpdf->n + 2) . " 0 R >> >>");
            }
            $this->mpdf->_out(
                    '/Opt [ ' . $this->mpdf->_textstring($form['OPT']) . ' ' .
                             $this->mpdf->_textstring($form['OPT']) . ' ]');
        }
        
        if ($form['subtype'] == 'radio')
        {
            if ($form['disabled'] ||
                     $this->form_radio_groups[$form['T']]['disabled'])
            {
                $radio_color = '0.5 0.5 0.5';
                $radio_background_color = '0.9 0.9 0.9';
            }
            ...
            $this->mpdf->_out(
                    '/Parent ' . $this->form_radio_groups[$form['T']]['obj_id'] .
                             ' 0 R ');
            $temp = '';
            $temp .= '/BC [ ' . $radio_color . " ] ";
            $temp .= '/BG [ ' . $radio_background_color . " ] ";
            ...
            $form['FF'][] = 15; // NoToggleOff - must be same as radio button
                                // group setting?
            $this->mpdf->_out('/Ff ' . $this->_setflag($form['FF']));
            if ($this->formUseZapD) $this->mpdf->_out(
                    '/DA (/F' . $this->mpdf->fonts['czapfdingbats']['i'] .
                             ' 0 Tf ' . $radio_color . ' rg)');
            else $this->mpdf->_out(
                    '/DA (/F' .
        ...
        {
            $temp .= $form['CA'] ? '/CA ' . $this->mpdf->_textstring(
                    $form['CA']) . ' ' : '/CA ' .
                     $this->mpdf->_textstring($form['T']) . ' ';
            $temp .= $form['RC'] ? '/RC ' . $this->mpdf->_textstring(
                    $form['RC']) . ' ' : '/RC ' .
                     $this->mpdf->_textstring($form['T']) . ' ';
            $temp .= $form['AC'] ? '/AC ' . $this->mpdf->_textstring(
                    $form['AC']) . ' ' : '/AC ' .
                     $this->mpdf->_textstring($form['T']) . ' ';
            $this->mpdf->_out("/BS << $bstemp >>");
            $this->mpdf->_out('/MK << ' . $temp . ' >>');
            $this->mpdf->_out(
                    '/DA (/F' . $this->mpdf->fonts[$form['style']['font']]['i'] .
                             ' ' . $form['style']['fontsize'] . ' Tf ' .
                             $form['style']['fontcolor'] . ')');
        ...
        if ($form['subtype'] == 'submit')
        {
            ...
            // To submit a value, needs to be in /AP dictionary, AND this object
            // must contain a /Fields entry
            // listing all fields to output
            $this->mpdf->_out(
                    '/AA << /D << /S /SubmitForm /F (' . $form['URL'] .
                             ') /Flags ' . $flag . ' >> >>');
            $form['FF'][] = 17;
            $this->mpdf->_out('/Ff ' . $this->_setflag($form['FF']));
        }
        ...
        if ($form['subtype'] == 'js_button')
        {
            ...
            if (isset($this->form_button_icon[$form['T']]))
            {
                ...
                $temp .= '/IX ' . ($cc + $this->mpdf->n) . ' 0 R '; // onClick /
                                                                // onMouseDown
                $temp .= '/IF << /SW /A /S /A /A [0.0 0.0] >> '; // Icon fit
                                                                 // dictionary
                if ($this->form_button_icon[$form['T']]['Indexed'])
                {
                    $cc ++;
            ...
            {
                $cc ++;
                $this->mpdf->_out("/AA << /D " . ($cc + $this->mpdf->n) .
                         " 0 R >>");
                $put_js = 1;
            }
        }
        ...
        if ($put_icon == 1)
        {
            $this->_put_button_icon($this->form_button_icon[$form['T']], 
                    $form['w'], $form['h']);
            $put_icon = NULL;
        }
        // obj + 1
        if ($put_js == 1)
        {
            $this->mpdf->_set_object_javascript(
                    $this->array_form_button_js[$form['T']]['js']);
        ...
        if ($form['subtype'] == 'radio')
        {
            ...
            if ($this->formUseZapD)
            {
                ...
                $fi = 'czapfdingbats';
                $r_on = 'q ' . $radio_color . ' rg BT /F' .
                         $this->mpdf->fonts[$fi]['i'] . ' ' . $fs .
                         ' Tf 0 0 Td (4) Tj ET Q';
                $r_off = 'q ' . $radio_color . ' rg BT /F' .
                         $this->mpdf->fonts[$fi]['i'] . ' ' . $fs .
                         ' Tf 0 0 Td (8) Tj ET Q';
            }
            else
            {
                $matrix = sprintf('%.3F 0 0 %.3F 0 %.3F', 
                        $form['style']['fontsize'] * 1.33 / 10, 
                        $form['style']['fontsize'] * 1.25 / 10, 
                        $form['style']['fontsize']);
                ...
                $r_on = 'q ' . $matrix . ' cm ' . $fill . $radio_color . ' rg ' .
                         $circle . '  ' . $radio_color . ' rg
...
f Q ';
                $r_off = 'q ' . $matrix . ' cm ' . $fill . $radio_color . ' rg ' .
                         $circle . '  Q ';
            }
        ...
        if ($form['subtype'] == 'checkbox')
        {
            ...
            if ($this->formUseZapD)
            {
                $fs = sprintf('%.3F', $form['style']['fontsize'] * 1.25);
                $fi = 'czapfdingbats';
                $cb_on = 'q ' . $radio_color . ' rg BT /F' .
                         $this->mpdf->fonts[$fi]['i'] . ' ' . $fs .
                         ' Tf 0 0 Td (4) Tj ET Q';
                $cb_off = 'q ' . $radio_color . ' rg BT /F' .
                         $this->mpdf->fonts[$fi]['i'] . ' ' . $fs .
                         ' Tf 0 0 Td (8) Tj ET Q';
            }
            else
            {
                $matrix = sprintf('%.3F 0 0 %.3F 0 %.3F', 
                        ...
                        $form['style']['fontsize']);
                $fill = $radio_background_color .
                         ' rg 7.395 -0.070 m 7.395 -7.344 l 0.121 -7.344 l 0.121 -0.070 l 7.395 -0.070 l h  f ';
                ...
                $cb_on = 'q ' . $matrix . ' cm ' . $fill . $radio_color . ' rg ' .
                         $square . ' f ' . $radio_color .
                         ' rg
6.321 -1.352 m 5.669 -2.075 5.070 -2.801 4.525 -3.532 c 3.979 -4.262 3.508 -4.967 3.112 -5.649 c 3.080 -5.706 3.039 -5.779 2.993 -5.868 c 2.858 -6.118 2.638 -6.243 2.334 -6.243 c 2.194 -6.243 2.100 -6.231 2.052 -6.205 c 2.003 -6.180 1.954 -6.118 1.904 -6.020 c 1.787 -5.788 1.688 -5.523 1.604 -5.226 c 1.521 -4.930 1.480 -4.721 1.480 -4.600 c 1.480 -4.535 1.491 -4.484 1.512 -4.447 c 1.535 -4.410 1.579 -4.367 1.647 -4.319 c 1.733 -4.259 1.828 -4.210 1.935 -4.172 c 2.040 -4.134 2.131 -4.115 2.205 -4.115 c 2.267 -4.115 2.341 -4.232 2.429 -4.469 c 2.437 -4.494 2.444 -4.511 2.448 -4.522 c 2.451 -4.531 2.456 -4.546 2.465 -4.568 c 2.546 -4.795 2.614 -4.910 2.668 -4.910 c 2.714 -4.910 2.898 -4.652 3.219 -4.136 c 3.539 -3.620 3.866 -3.136 4.197 -2.683 c 4.426 -2.367 4.633 -2.103 4.816 -1.889 c 4.998 -1.676 5.131 -1.544 5.211 -1.493 c 5.329 -1.426 5.483 -1.368 5.670 -1.319 c 5.856 -1.271 6.066 -1.238 6.296 -1.217 c 6.321 -1.352 l h  f  Q ';
                $cb_off = 'q ' . $matrix . ' cm ' . $fill . $radio_color . ' rg ' .
                         $square . ' f Q ';
            }
            $this->mpdf->_newobj();
            ...
            // output appearance stream for check box off (only if not using
            // ZapfDingbats)
            if (! $this->formUseZapD)
            {
                $this->mpdf->_newobj();
    ...
    function _putform_ch ($form, $hPt)
    {
        ...
        {
            $opt .= '[ ' . $this->mpdf->_textstring($form['OPT']['VAL'][$i]) .
                     ' ' . $this->mpdf->_textstring($form['OPT']['OPT'][$i]) .
                     ' ] ';
        }
        ...
        {
            $selectName = $this->mpdf->_textstring(
                    $form['OPT']['VAL'][$selectVal]);
        ...
        {
            $this->mpdf->_set_object_javascript(
                    $this->array_form_choice_js[$form['T']]['js']);
    ...
    function _putform_tx ($form, $hPt)
    {
        ...
        $this->mpdf->_out(
                '/Rect [ ' .
                         $this->_form_rect($form['x'], $form['y'], $form['w'], 
                                $form['h'], $hPt) . ' ] ');
        $form['hidden'] ? $this->mpdf->_out('/F 2 ') : $this->mpdf->_out(
                '/F 4 ');
        ...
        $this->mpdf->_out('/T ' . $this->mpdf->_textstring($form['T']));
        $this->mpdf->_out('/TU ' . $this->mpdf->_textstring($form['TU']));
        if ($form['V'] || $form['V'] === '0') $this->mpdf->_out(
                '/V ' . $this->mpdf->_textstring($form['V']));
        ...
        if ($put_js == 1)
        {
            ...
            {
                $this->mpdf->_set_object_javascript(
                        $this->array_form_text_js[$form['T']]['F']['js']);
                unset($this->array_form_text_js[$form['T']]['F']);
            }
            if (isset($this->array_form_text_js[$form['T']]['K']))
            {
                $this->mpdf->_set_object_javascript(
                        $this->array_form_text_js[$form['T']]['K']['js']);
                unset($this->array_form_text_js[$form['T']]['K']);
            }
            if (isset($this->array_form_text_js[$form['T']]['V']))
            {
                $this->mpdf->_set_object_javascript(
                        $this->array_form_text_js[$form['T']]['V']['js']);
                unset($this->array_form_text_js[$form['T']]['V']);
            }
            if (isset($this->array_form_text_js[$form['T']]['C']))
            {
                $this->mpdf->_set_object_javascript(
                        $this->array_form_text_js[$form['T']]['C']['js']);
