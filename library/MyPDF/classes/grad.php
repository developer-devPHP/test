...
class grad
{
    ...
    // mPDF 5.3.A1
    function CoonsPatchMesh ($x, $y, $w, $h, $patch_array = array(), $x_min = 0, 
            $x_max = 1, $y_min = 0, $y_max = 1, $colspace = 'RGB', $return = false)
    {
        $s = ' q ';
        $s .= sprintf(' %.3F %.3F %.3F %.3F re W n ', $x * _MPDFK, 
                ($this->mpdf->h - $y) * _MPDFK, $w * _MPDFK, - $h * _MPDFK);
        $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w * _MPDFK, $h * _MPDFK, 
                $x * _MPDFK, ($this->mpdf->h - ($y + $h)) * _MPDFK);
        ...
        for ($i = 0; $i < count($patch_array); $i ++)
        {
            ...
            for ($j = 0; $j < count($patch_array[$i]['points']); $j ++)
            {
                ...
                if (($j % 2) == 1)
                { // Y coordinate (adjusted as input is From
                                     // top left)
                    $patch_array[$i]['points'][$j] = (($patch_array[$i]['points'][$j] -
                             $y_min) / ($y_max - $y_min)) * $bpcd;
                    $patch_array[$i]['points'][$j] = $bpcd -
                             $patch_array[$i]['points'][$j];
                }
                else
                {
                    $patch_array[$i]['points'][$j] = (($patch_array[$i]['points'][$j] -
                             $x_min) / ($x_max - $x_min)) * $bpcd;
                }
                if ($patch_array[$i]['points'][$j] < 0) $patch_array[$i]['points'][$j] = 0;
                if ($patch_array[$i]['points'][$j] > $bpcd) $patch_array[$i]['points'][$j] = $bpcd;
                $this->mpdf->gradients[$n]['stream'] .= chr(
                        floor($patch_array[$i]['points'][$j] / 256));
                $this->mpdf->gradients[$n]['stream'] .= chr(
                        floor($patch_array[$i]['points'][$j] % 256));
            }
            for ($j = 0; $j < count($patch_array[$i]['colors']); $j ++)
            {
                ...
                if ($colspace == 'RGB')
                {
                    ...
                    if (isset($patch_array[$i]['colors'][$j][4]) &&
                             ord($patch_array[$i]['colors'][$j][4]) < 100)
                    {
                        $trans = true;
                    }
                }
                    ...
                    {
                        $this->mpdf->gradients[$n]['stream'] .= chr(
                                ord($patch_array[$i]['colors'][$j][1]) * 2.55);
                        $this->mpdf->gradients[$n]['stream'] .= chr(
                                ord($patch_array[$i]['colors'][$j][2]) * 2.55);
                        $this->mpdf->gradients[$n]['stream'] .= chr(
                                ord($patch_array[$i]['colors'][$j][3]) * 2.55);
                        $this->mpdf->gradients[$n]['stream'] .= chr(
                                ord($patch_array[$i]['colors'][$j][4]) * 2.55);
                        if (isset($patch_array[$i]['colors'][$j][5]) &&
                                 ord($patch_array[$i]['colors'][$j][5]) < 100)
                        {
                            $trans = true;
                        }
                    }
        ...
        if ($trans)
        {
            ...
            for ($i = 0; $i < count($patch_array); $i ++)
            {
                $this->mpdf->gradients[$n]['stream_trans'] .= chr(
                        $patch_array[$i]['f']);
                for ($j = 0; $j < count($patch_array[$i]['points']); $j ++)
                {
                    // each point as 16 bit
    ...
    function Gradient ($x, $y, $w, $h, $type, $stops = array(), $colorspace = 'RGB', 
            $coords = '', $extend = '', $return = false, $is_mask = false)
    {
        ...
        if ($coords[0] !== false &&
                 preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $coords[0], 
                        $m))
        {
            $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
        ...
        if ($coords[1] !== false &&
                 preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $coords[1], 
                        $m))
        {
            $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                    $this->mpdf->FontSize, false);
        ...
        if ($type == 2)
        {
            ...
            // NO end points (Angle defined & Start points)
                ...
                if ($angle !== false && $coords[0] !== false &&
                 $coords[1] !== false && $coords[2] === false &&
                 $coords[3] === false)
        {
            if ($angle == 0 || $angle == 360)
            {
                $coords[3] = $coords[1];
                if ($coords[0] == 1) $coords[2] = 2;
                else $coords[2] = 1;
            }
            else 
                if ($angle == 90)
                {
                    $coords[2] = $coords[0];
                    $coords[3] = 1;
                    if ($coords[1] == 1) $coords[3] = 2;
                    else $coords[3] = 1;
                }
                else 
                    if ($angle == 180)
                    {
                        if ($coords[4] == 0) $coords[2] = - 1;
                        else $coords[2] = 0;
                        $coords[3] = $coords[1];
                    }
                    else 
        ...
        // axis goes through the box. The axis runs along the specified angle.
        // The end point of the axis is defined such that the
        // farthest corner of the box from the starting point is perpendicular
        // to the gradient axis at that point.
        // NO end points or Start points (Angle defined)
            ...
            // NO angle and NO end points (Start points defined)
                ...
                if ((! isset($angle) || $angle === false) && $coords[0] !== false &&
                         $coords[1] !== false)
                { // should
                                                                                                           // have
                                                                                                           // start
                ...
                { // default values T2B
                       // All values are set in parseMozGradient - so won't
                       // appear here
                    $coords = array(
                            0,
                            0,
                            1,
                            0
                    ); // default for original linear
                                              // gradient (L2R)
                }
    $s = ' q';
    $s .= sprintf(' %.3F %.3F %.3F %.3F re W n', $x * _MPDFK, 
            ($this->mpdf->h - $y) * _MPDFK, $w * _MPDFK, - $h * _MPDFK) . "\n";
    $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * _MPDFK, $useh * _MPDFK, 
            $usex * _MPDFK, ($this->mpdf->h - ($usey + $useh)) * _MPDFK) . "\n";
}
...
// RADIAL
    ...
    if ($type == 3)
    {
        ...
        $size = $coords[7];
        $repeat = $coords[8];
        // ALL POINTS AND RADIUS SET (default for custom mPDF radial gradient) -
        // no -moz
        if ($coords[0] !== false && $coords[1] !== false && $coords[2] !== false &&
                 $coords[3] !== false && $coords[4] !== false)
        {
            // do nothing - coords used as they are
        }
        // If a <point> is defined
            ...
            if ($shape !== false && $size !== false)
            {
                ...
                if ($shape == 'ellipse')
                {
                    ...
                    $corner2 = sqrt(pow($coords[0], 2) + pow((1 - $coords[1]), 
                            2));
                    $corner3 = sqrt(pow((1 - $coords[0]), 2) + pow($coords[1], 
                            2));
                    $corner4 = sqrt(
                            pow((1 - $coords[0]), 2) + pow((1 - $coords[1]), 2));
                    if ($size == 'closest-side')
                    {
                        ...
                        {
                            $radius = min($corner1, $corner2, $corner3, 
                                    $corner4);
                        }
                        else 
                            if ($size == 'farthest-side')
                            {
                                $radius = max($coords[0], $coords[1], 
                                        (1 - $coords[0]), (1 - $coords[1]));
                            }
                            else
                            {
                                $radius = max($corner1, $corner2, $corner3, 
                                        $corner4);
                            } // farthest
                                                                                            // corner
                                                                                            // (default)
                ...
                // CIRCLE
                    ...
                    if ($shape == 'circle')
                    {
                        if ($w >= $h)
                        {
                            ...
                            $corner1 = sqrt(
                                    pow($coords[0], 2) + pow($coords[1], 2));
                            $corner2 = sqrt(
                                    pow($coords[0], 2) +
                                             pow((($h / $w) - $coords[1]), 2));
                            $corner3 = sqrt(
                                    pow((1 - $coords[0]), 2) + pow($coords[1], 
                                            2));
                            $corner4 = sqrt(
                                    pow((1 - $coords[0]), 2) +
                                             pow((($h / $w) - $coords[1]), 2));
                            if ($size == 'closest-side')
                            {
                                $radius = min($coords[0], $coords[1], 
                                        (1 - $coords[0]), (($h / $w) - $coords[1]));
                            }
                            else 
                                if ($size == 'closest-corner')
                                {
            ...
            { // default values
                   // All values are set in parseMozGradient - so won't appear
                   // here
                $coords = array(
                        0.5,
                        0.5,
        ...
        $s .= sprintf(' %.3F %.3F %.3F %.3F re W n', $x * _MPDFK, 
                ($this->mpdf->h - $y) * _MPDFK, $w * _MPDFK, - $h * _MPDFK) . "\n";
        $s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * _MPDFK, 
                $useh * _MPDFK, $usex * _MPDFK, 
...
} // Absolute lengths are meaningless
                                          // for an ellipse - Firefox uses Width
                                          // as reference

for ($i = 0; $i < count($stops); $i ++)
{
    if (isset($stops[$i]['offset']) &&
             preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', 
                    $stops[$i]['offset'], $m))
    {
        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                $this->mpdf->FontSize, false);
...
if (! $repeat && isset($stops[(count($stops) - 1)]['offset']) &&
         $stops[(count($stops) - 1)]['offset'] < 1)
{
...
for ($i = 0; $i < count($stops); $i ++)
{
    ...
    if ($colorspace == 'CMYK')
    {
        $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf(
                '%.3F %.3F %.3F %.3F', (ord($stops[$i]['col']{1}) / 100), 
                (ord($stops[$i]['col']{2}) / 100), 
                (ord($stops[$i]['col']{3}) / 100), 
                (ord($stops[$i]['col']{4}) / 100));
    }
    else 
        if ($colorspace == 'Gray')
        {
            $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf('%.3F', 
                    (ord($stops[$i]['col']{1}) / 255));
        }
        else
        {
            $this->mpdf->gradients[$n]['stops'][$i]['col'] = sprintf(
                    '%.3F %.3F %.3F', (ord($stops[$i]['col']{1}) / 255), 
                    (ord($stops[$i]['col']{2}) / 255), 
    ...
    else
    {
        ...
        for ($f = 0; $f < ($j - $i - 1); $f ++)
        {
            $stops[($i + $f)]['offset'] = $stops[($i + $f - 1)]['offset'] +
                     ($int);
        }
    }
}
...
for ($i = 0; $i < $ns; $i ++)
{
    $this->mpdf->gradients[$n]['stops'][(($ns * $gp) + $i)] = $this->mpdf->gradients[$n]['stops'][(($ns *
             ($gp - 1)) + $i)];
    $tmp = $this->mpdf->gradients[$n]['stops'][(($ns * ($gp - 1)) + ($ns - 1))]['offset'] +
             $offs[$i];
    if ($tmp < 1)
    {
...
$extend = array(
    'true',
    'true'
); // These are supposed to be quoted - appear in
                               // PDF file as text
}
...
if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i', $bgr[0]) &&
     ! preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0]))
{
$startStops = 1;
}
else 
...
if ($startStops == 1)
{ // default values
                        // [<point> || <angle>,] = [<%// [<point> || <angle>,] = [<% em px left center right
                        // bottom top> || <deg grad rad 0>,]
...
if (preg_match('/(\d+)[%]/i', $first[0], $m))
{
    ...
    if (! isset($startx) &&
             preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[0], $m))
    {
        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                $this->mpdf->FontSize, false);
        if ($tmp)
...
if (isset($first[1]) && preg_match('/(\d+)[%]/i', $first[1], $m))
{
    ...
    if (! isset($starty) &&
             preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[1], $m))
    {
        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                $this->mpdf->FontSize, false);
        if ($tmp)
...
// If neither a <point> or <angle> is specified, i.e. the entire function
// consists of only <stop> values, the gradient axis starts from the top of the
// box and runs vertically downwards, ending at the bottom of the box.
else
{ // default values T2B
$starty = 1;
$startx = 0.5;
...
if ($col{0} == 1) $g['colorspace'] = 'Gray';
    ...
    if ($col{0} == 4 || $col{0} == 6) $g['colorspace'] = 'CMYK';
if ($col{0} == 5)
{
...
if (isset($el[1]) && preg_match('/(\d+)[%]/', $el[1], $m))
{
    ...
    if (isset($el[1]) &&
             preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $el[1], $m))
    {
        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                $this->mpdf->FontSize, false);
        if ($tmp)
         ...
         ! preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0]))
{
    ...
    if (trim($first[(count($first) - 1)]) === "0")
    {
        ...
        if (preg_match(
                '/(circle|ellipse|closest-side|closest-corner|farthest-side|farthest-corner|contain|cover)/i', 
                $bgr[0]))
        {
            $startStops = 1;
            $shape_size = $bgr[0];
...
if ($pos_angle)
{ // default values
                  // [<point> || <angle>,] = [<%// [<point> || <angle>,] = [<% em px left center right bottom
                  // top> || <deg grad rad 0>,]
    ...
    if (preg_match('/(\d+)[%]/i', $first[0], $m))
    {
        ...
        if (! isset($startx) &&
                 preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[0], 
                        $m))
        {
            $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                    $this->mpdf->FontSize, false);
    ...
    if (isset($first[1]) && preg_match('/(\d+)[%]/i', $first[1], $m))
    {
        ...
        if (! isset($starty) &&
                 preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[1], 
                        $m))
        {
            $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, 
                    $this->mpdf->FontSize, false);
    ...
    /*
     * // ?? Angle has no effect in radial gradient (does not exist in CSS3
     * spec.) if (preg_match('/([\-]*[0-9\.]+)(deg|grad|rad)/i',$pos_angle,$m))
     * { $angle = $m[1] + 0; if (strtolower($m[2])=='deg') { $angle = $angle; }
     * else if (strtolower($m[2])=='grad') { $angle *= (360/400); } else if
     * (strtolower($m[2])=='rad') { $angle = rad2deg($angle); } while($angle <
     * 0) { $angle += 360; } $angle = ($angle % 360); }
     */
    if (! isset($starty))
    {
        $starty = 0.5;
...
{ // default values
    ...
    if (preg_match(
            '/(closest-side|closest-corner|farthest-side|farthest-corner|contain|cover)/i', 
            $shape_size, $m))
    {
        $size = $m[1];
        if ($size == 'contain')
...
if (count($bgr) > 6)
{
if (strtoupper(substr($bgr[0], 0, 1)) == 'L' && count($bgr) == 7)
{ // linear
$g['type'] = 2;
// $coords = array(0,0,1,1 ); // 0 0 1 0 or 0 1 1 1 is L 2 R; 1,1,0,1 is R2L;
// 1,1,1,0 is T2B; 1,0,1,1 is B2T
// Linear: $coords - array of the form (x1, y1, x2, y2) which defines the
// gradient vector (see linear_gradient_coords.jpg).
// The default value is from left to right (x1=0, y1=0, x2=1, y2=0).
$g['coords'] = array(
...
if ($cor{0} == 4 || $cor{0} == 6) $g['colorspace'] = 'CMYK';
if ($cor)
{
$g['col'] = $cor;
}
