...
class TTFontFile
{
    ...
    function TTFontFile ()
    {
        $this->maxStrLenRead = 200000; // Maximum size of glyf table to read in
                                       // as string (otherwise reads each glyph
                                       // from file)
    }

    function getMetrics ($file, $TTCfontID = 0, $debug = false, $BMPonly = false, 
            $kerninfo = false)
    {
        $this->filename = $file;
        ...
        $this->panose = array();
        if ($version == 0x4F54544F) die("Postscript outlines are not supported");
        if ($version == 0x74746366 && ! $TTCfontID) die(
                "ERROR - You must define the TTCfontID for a TrueType Collection in config_fonts.php (" .
                         $file . ")");
        if (! in_array($version, array(
                0x00010000,
                0x74727565
        )) && ! $TTCfontID) die("Not a TrueType font: version=" . $version);
        if ($TTCfontID > 0)
        {
            $this->version = $version = $this->read_ulong(); // TTC Header
                                                             // version now
            if (! in_array($version, array(
                    0x00010000,
                    0x00020000
            ))) die(
                    "ERROR - Error parsing TrueType Collection: version=" .
                             $version . " - " . $file);
            ...
            $this->version = $version = $this->read_ulong(); // TTFont version
                                                             // again now
        }
    ...
    function checksumTables ()
    {
        ...
        foreach ($this->tables as $t)
        {
            ...
            { // 1.02
                ...
                if ($xchecksum != $checksum) die(
                        sprintf(
                                'TTF file "%s": invalid checksum %s table: %s (expected %s)', 
                                $this->filename, 
                                dechex($checksum[0]) . dechex($checksum[1]), 
                                $t['tag'], 
    ...
    function read_ulong ()
    {
        ...
        return (ord($s[0]) * 16777216) + (ord($s[1]) << 16) + (ord($s[2]) << 8) +
                 ord($s[3]); // 16777216
                                                                                                // =
                                                                                                // 1<<24
    }
    ...
    {
        return substr($stream, 0, $offset) . $value .
    ...
    function getTTCFonts ($file)
    {
        ...
        if ($version == 0x74746366)
        {
            ...
            if (! in_array($version, array(
                    0x00010000,
                    0x00020000
            ))) return ("ERROR - Error parsing TrueType Collection: version=" .
                     $version . " - " . $file);
        }
        else
        {
            return ("ERROR - Not a TrueType Collection: version=" . $version .
                     " - " . $file);
        }
        $this->numTTCFonts = $this->read_ulong();
        for ($i = 1; $i <= $this->numTTCFonts; $i ++)
    ...
    function extractCoreInfo ($file, $TTCfontID = 0)
    {
        ...
        if ($version == 0x4F54544F) return ("ERROR - NOT ADDED as Postscript outlines are not supported - " .
                 $file);
        if ($version == 0x74746366)
        {
            if ($TTCfontID > 0)
            {
                ...
                if (! in_array($version, array(
                        0x00010000,
                        0x00020000
                ))) return ("ERROR - NOT ADDED as Error parsing TrueType Collection: version=" .
                         $version . " - " . $file);
            }
            else
        ...
        else
        {
            if (! in_array($version, array(
                    0x00010000,
                    0x74727565
            ))) return ("ERROR - NOT ADDED as Not a TrueType font: version=" .
                     $version . " - " . $file);
            $this->readTableDirectory(false);
        }
        
        /*
         * Included for testing... $cmap_offset = $this->seek_table("cmap");
         * $this->skip(2); $cmapTableCount = $this->read_ushort();
         * $unicode_cmap_offset = 0; for ($i=0;$i<$cmapTableCount;$i++) {
         * $x[$i]['platformId'] = $this->read_ushort(); $x[$i]['encodingId'] =
         * $this->read_ushort(); $x[$i]['offset'] = $this->read_ulong();
         * $save_pos = $this->_pos; $x[$i]['format'] =
         * $this->get_ushort($cmap_offset + $x[$i]['offset'] );
         * $this->seek($save_pos ); } print_r($x); exit;
         */
        // /////////////////////////////////
        // name - Naming table
        // /////////////////////////////////
        ...
        return ("ERROR - NOT ADDED as Unknown name table format " . $format .
                 " - " . $file);
        ...
        for ($i = 0; $i < $numRecords; $i ++)
        {
            ...
            { // Microsoft,
                ...
                { // Macintosh,
                                                                                     ...
                                                                                     // Name
                    $opos = $this->_pos;
                    $N = $this->get_chunk($string_data_offset + $offset, 
                            $length);
                    $this->_pos = $opos;
                    $this->seek($opos);
                }
        ...
        if ($names[6]) $psName = preg_replace('/ /', '-', $names[6]);
        else 
            if ($names[4]) $psName = preg_replace('/ /', '-', $names[4]);
            else 
                if ($names[1]) $psName = preg_replace('/ /', '-', $names[1]);
                else $psName = '';
        if (! $names[1] && ! $psName) return ("ERROR - NOT ADDED as Could not find valid font name - " .
                 $file);
        $this->name = $psName;
        if ($names[1])
        {
        ...
        if ($ver_maj != 1) return ('ERROR - NOT ADDED as Unknown head table version ' .
                 $ver_maj . '.' . $ver_min . " - " . $file);
        $this->fontRevision = $this->read_ushort() . $this->read_ushort();
        $this->skip(4);
        $magic = $this->read_ulong();
        if ($magic != 0x5F0F3CF5) return ('ERROR - NOT ADDED as Invalid head table magic ' .
                 $magic . " - " . $file);
        $this->skip(2);
        ...
        for ($i = 0; $i < $cmapTableCount; $i ++)
        {
            ...
            { // Microsoft,
                ...
                if ($format == 4)
                {
                    if (! $unicode_cmap_offset) $unicode_cmap_offset = $cmap_offset +
                             $offset;
                }
            }
            else 
        ...
        if (! $unicode_cmap_offset) return ('ERROR - Font (' . $this->filename .
        ...
        // Format 12 CMAP does characters above Unicode BMP i.e. some HKCS
        // characters U+20000 and above
        if ($format == 12)
        {
            ...
            for ($i = 0; $i < $nGroups; $i ++)
            {
                ...
                if (($endCharCode > 0x20000 && $endCharCode < 0x2A6DF) ||
                         ($endCharCode > 0x2F800 && $endCharCode < 0x2FA1F))
                {
                    $sip = true;
                }
                else 
                    if ($endCharCode > 0x10000 && $endCharCode < 0x1FFFF)
                    {
                        ...
                        if (($endCharCode > 0x0590 && $endCharCode < 0x077F) ||
                                 ($endCharCode > 0xFE70 && $endCharCode < 0xFEFF) ||
                                 ($endCharCode > 0xFB50 && $endCharCode < 0xFDFF))
                        {
                            $rtl = true;
                        }
                        else 
        ...
        { // Format 4 CMap
            ...
            {
                if (($endCount[$n] > 0x0590 && $endCount[$n] < 0x077F) ||
                         ($endCount[$n] > 0xFE70 && $endCount[$n] < 0xFEFF) ||
                         ($endCount[$n] > 0xFB50 && $endCount[$n] < 0xFDFF))
                {
                    $rtl = true;
                }
                else 
                    if ($endCount[$n] > 0x0900 && $endCount[$n] < 0x0DFF)
                    {
                        ...
                        if (($endCount[$n] > 0x2E80 && $endCount[$n] < 0x4DC0) ||
                                 ($endCount[$n] > 0x4E00 &&
                                 $endCount[$n] < 0xA4CF) ||
                                 ($endCount[$n] > 0xAC00 &&
                                 $endCount[$n] < 0xD7AF) ||
    ...
    function extractInfo ($debug = false, $BMPonly = false, $kerninfo = false)
    {
        ...
        /*
         * Test purposes - displays table of names $name_offset =
         * $this->seek_table("name"); $format = $this->read_ushort(); if
         * ($format != 0 && $format != 1)	// mPDF 5.3.73 die("Unknown name table
         * format ".$format); $numRecords = $this->read_ushort();
         * $string_data_offset = $name_offset + $this->read_ushort(); for
         * ($i=0;$i<$numRecords; $i++) { $x[$i]['platformId'] =
         * $this->read_ushort(); $x[$i]['encodingId'] = $this->read_ushort();
         * $x[$i]['languageId'] = $this->read_ushort(); $x[$i]['nameId'] =
         * $this->read_ushort(); $x[$i]['length'] = $this->read_ushort();
         * $x[$i]['offset'] = $this->read_ushort(); $N = ''; if
         * ($x[$i]['platformId'] == 1 && $x[$i]['encodingId'] == 0 &&
         * $x[$i]['languageId'] == 0) { // Roman $opos = $this->_pos; $N =
         * $this->get_chunk($string_data_offset + $x[$i]['offset'] ,
         * $x[$i]['length'] ); $this->_pos = $opos; $this->seek($opos); } else {
         * 	// Unicode $opos = $this->_pos; $this->seek($string_data_offset +
         * $x[$i]['offset'] ); $length = $x[$i]['length'] ; if ($length % 2 !=
         * 0) $length -= 1; //		die("PostScript name is UTF-16BE string of odd
         * length"); $length /= 2; $N = ''; while ($length > 0) { $char =
         * $this->read_ushort(); $N .= (chr($char)); $length -= 1; } $this->_pos
         * = $opos; $this->seek($opos); } $x[$i]['names'][$nameId] = $N; }
         * print_r($x); exit;
         */
 ...
 = $names[6];
        ...
        if (! $psName) die(
                "Could not find PostScript font name: " . $this->filename);
        if ($debug)
        {
            ...
            { // mPDF 5.3.07
                $c = $psName[$i];
                $oc = ord($c);
                if ($oc > 126 || strpos(' [](){}<>/%', $c) !== false) die(
                        "psName=" . $psName . " contains invalid character " . $c .
                                 " ie U+" . ord(c));
            }
        }
        $this->name = $psName;
        ...
        if ($debug)
        {
            ...
            $ver_min = $this->read_ushort();
            if ($ver_maj != 1) die(
                    'Unknown head table version ' . $ver_maj . '.' . $ver_min);
            $this->fontRevision = $this->read_ushort() . $this->read_ushort();
            
            $this->skip(4);
            $magic = $this->read_ulong();
            if ($magic != 0x5F0F3CF5) die('Invalid head table magic ' . $magic);
            $this->skip(2);
        }
        else
        {
        ...
        if ($glyphDataFormat != 0) die(
                'Unknown glyph data format ' . $glyphDataFormat);
        
        // /////////////////////////////////
        // hhea metrics table
        // /////////////////////////////////
        // ttf2t1 seems to use this value rather than the one in OS/2 - so put
        ...
        if (isset($this->tables["OS/2"]))
        {
            ...
            if ($fsType == 0x0002 || ($fsType & 0x0300) != 0)
            {
                ...
                if (! $overrideTTFFontRestriction) die(
        ...
        if ($debug)
        {
            $ver_maj = $this->read_ushort();
            $ver_min = $this->read_ushort();
            if ($ver_maj < 1 || $ver_maj > 4) die(
                    'Unknown post table version ' . $ver_maj);
        }
        else
        {
            $this->skip(4);
        ...
        if ($debug)
        {
            $ver_maj = $this->read_ushort();
            $ver_min = $this->read_ushort();
            if ($ver_maj != 1) die('Unknown hhea table version ' . $ver_maj);
            $this->skip(28);
        }
        else
        {
        ...
        if ($debug)
        {
            $ver_maj = $this->read_ushort();
            $ver_min = $this->read_ushort();
            if ($ver_maj != 1) die('Unknown maxp table version ' . $ver_maj);
        }
        else
        {
            $this->skip(4);
        ...
        // /////////////////////////////////
        /*
         * Test purposes - displays list of cmaps $cmap_offset =
         * $this->seek_table("cmap"); $this->skip(2); $cmapTableCount =
         * $this->read_ushort(); $unicode_cmap_offset = 0; for
         * ($i=0;$i<$cmapTableCount;$i++) { $x[$i]['platformId'] =
         * $this->read_ushort(); $x[$i]['encodingId'] = $this->read_ushort();
         * $x[$i]['offset'] = $this->read_ulong(); $save_pos = $this->_pos;
        ...
        if ($kerninfo)
        {
            // Recognises old form of Kerning table - as required by Windows -
            // Format 0 only
            ...
            if ($kern_offset && $version == 0 && $format == 0)
            {
                ...
                for ($i = 0; $i < $nPairs; $i ++)
                {
                    ...
                    $val = $this->read_short();
                    if (count($glyphToChar[$left]) == 1 &&
                             count($glyphToChar[$right]) == 1)
                    {
                        if ($left != 32 && $right != 32)
                        {
                            $this->kerninfo[$glyphToChar[$left][0]][$glyphToChar[$right][0]] = intval(
                                    $val * $scale);
                        }
                    }
                }
    ...
    function makeSubset ($file, &$subset, $TTCfontID = 0, $debug = false)
    {
        ...
        foreach ($subset as $code)
        {
            ...
            {
                $subsetglyphs[$this->charToGlyph[$code]] = $code; // Old Glyph
                                                                  // ID =>
                                                                  // Unicode
                $subsetCharToGlyph[$code] = $this->charToGlyph[$code]; // Unicode
                                                                       // to old
                                                                       // GlyphID
            }
        ...
        $n = 0;
        // mPDF 5.0
        $fsLastCharIndex = 0; // maximum Unicode index (character code) in this
                              // font, according to the cmap subtable for
                              // platform ID 3 and platform- specific encoding
                              // ID 0 or 1.
        ...
        $segCount = count($range) + 1; // + 1 Last segment has missing character
                                       // 0xFFFF
        $searchRange = 1;
        $entrySelector = 0;
        ...
        $cmap = array(
                0,
                3, // Index : version, number of encoding
                                    // subtables
                0,
                ...
                4,
                $length,
                0, // Format 4 Mapping subtable: format, length,
                               // language
                $segCount * 2,
                $searchRange,
                $entrySelector,
        ...
        {
            $cmap[] = 0; // idRangeOffset[segCount] Offset in bytes to glyph
                         // indexArray, or 0
        }
        ...
        $maxComponentDepth = 0; // levels of recursion, set to 0 if font has
                                // only simple glyphs
        ...
        foreach ($subsetglyphs as $originalGlyphIdx => $uni)
        {
            ...
            {
                if ($glyphLen > 0) $data = $this->get_chunk(
                        $glyfOffset + $glyphPos, $glyphLen);
                else $data = '';
            }
            ...
            if ($glyphLen > 0)
            {
                if (_RECALC_PROFILE)
                {
                    ...
                    $minRightSideBearing = min($minRightSideBearing, 
                            ($aw - $lsb - ($xMax - $xMin)));
            ...
            { // If number of
                ...
                while ($flags & GF_MORE)
                {
                    $nComponentElements += 1; // number of glyphs referenced at
                                              // top level
                    ...
                    $data = $this->_set_ushort($data, $pos_in_glyph + 2, 
                            $glyphSet[$glyphIdx]);
                    $pos_in_glyph += 4;
                    if ($flags & GF_WORDS)
                    {
            ...
            // mPDF 5.0 - Simple Glyph
                ...
                if (_RECALC_PROFILE && $glyphLen > 2 && $up[1] < (1 << 15) &&
                         $up[1] > 0)
                { // Number
                                                                                                 // of
                                                                                                 // contours
                    ...
                    $pos_in_glyph = 10 + ($nContours * 2) - 2; // Last
                                                                // endContourPoint
                    $up = unpack("n", substr($data, $pos_in_glyph, 2));
                    $points = $up[1] + 1;
        ...
        if (_RECALC_PROFILE)
        {
            foreach ($this->glyphdata as $originalGlyphIdx => $val)
            {
                ...
                $points = 0;
                $contours = 0;
                $this->getGlyphData($originalGlyphIdx, $maxdepth, $depth, 
                        $points, $contours);
        ...
        if (_RECALC_PROFILE)
        {
            $head = $this->_set_short($head, 36, $xMinT); // for all glyph
                                                          // bounding boxes
            $head = $this->_set_short($head, 38, $yMinT); // for all glyph
                                                          // bounding boxes
            $head = $this->_set_short($head, 40, $xMaxT); // for all glyph
                                                          // bounding boxes
            $head = $this->_set_short($head, 42, $yMaxT); // for all glyph
                                                          // bounding boxes
                                                          // mPDF 5.3.33
            $head[17] = chr($head[17] & ~ (1 << 4)); // Unset Bit 4 (as hdmx/LTSH
                                                    // tables not included)
        }
        $this->add('head', $head);
        ...
        if (_RECALC_PROFILE)
        {
            $maxp = $this->_set_ushort($maxp, 6, $maxPoints); // points in
                                                              // non-compound
                                                              // glyph
            $maxp = $this->_set_ushort($maxp, 8, $maxContours); // contours in
                                                                // non-compound
                                                                // glyph
            $maxp = $this->_set_ushort($maxp, 10, $maxComponentPoints); // points
                                                                        // in
                                                                        // compound
                                                                        // glyph
            $maxp = $this->_set_ushort($maxp, 12, $maxComponentContours); // contours
                                                                          // in
                                                                          // compound
                                                                          // glyph
            $maxp = $this->_set_ushort($maxp, 28, $maxComponentElements); // number
                                                                          // of
                                                                          // glyphs
                                                                          // referenced
                                                                          // at
                                                                          // top
                                                                          // level
            $maxp = $this->_set_ushort($maxp, 30, $maxComponentDepth); // levels
                                                                       // of
                                                                       // recursion,
                                                                       // set to
                                                                       // 0 if
        ...
        if (isset($this->tables['OS/2']))
        {
            ...
            if (_RECALC_PROFILE)
            {
                ...
                $fsSelection = ($fsSelection & ~ (1 << 6)); // 2-byte bit field
                                                           // containing
                                                           // information
                                                           // concerning the
                                                           // nature of the font
            ...
            $nonBMP = ($nonBMP & ~ (1 << 9)); // Unset Bit 57 (indicates non-BMP)
            ...
            if (_RECALC_PROFILE)
            {
                ...
                $os2 = $this->_set_ushort($os2, 42, 0x0000); // ulCharRange
                                                             // (ulUnicodeRange)
                                                             // bits 24-31 |
                                                             // 16-23
                $os2 = $this->_set_ushort($os2, 44, 0x0000); // ulCharRange
                                                             // (Unicode ranges)
                                                             // bits 8-15 | 0-7
                $os2 = $this->_set_ushort($os2, 46, $nonBMP); // ulCharRange
                                                              // (Unicode
                                                              // ranges) bits
                                                              // 56-63 | 48-55
                $os2 = $this->_set_ushort($os2, 48, 0x0000); // ulCharRange
                                                             // (Unicode ranges)
                                                             // bits 40-47 |
                                                             // 32-39
                $os2 = $this->_set_ushort($os2, 50, 0x0000); // ulCharRange
                                                             // (Unicode ranges)
                                                             // bits 88-95 |
                                                             // 80-87
                $os2 = $this->_set_ushort($os2, 52, 0x0000); // ulCharRange
                                                             // (Unicode ranges)
                                                             // bits 72-79 |
                                                             // 64-71
                $os2 = $this->_set_ushort($os2, 54, 0x0000); // ulCharRange
                                                             // (Unicode ranges)
                                                             // bits 120-127 |
                                                             // 112-119
                $os2 = $this->_set_ushort($os2, 56, 0x0000); // ulCharRange
                                                             // (Unicode ranges)
                                                             // bits 104-111 |
                                                             // 96-103
            }
            $os2 = $this->_set_ushort($os2, 46, $nonBMP); // Unset Bit 57
                                                          // (indicates non-BMP)
                                                          // - for interactive
                                                          // forms
    ...
    function makeSubsetSIP ($file, &$subset, $TTCfontID = 0, $debug = false)
    {
        ...
        foreach ($subset as $code)
        {
            if (isset($charToGlyph[$code])) $originalGlyphIdx = $charToGlyph[$code];
            else 
                if ($code < 128)
                { // mPDF 5.0.067
        ...
        while ($n < count($glyphMap))
        {
            ...
            if ($numberOfContours < 0)
            {
                ...
                while ($flags & GF_MORE)
                {
                    ...
                    if ($flags & GF_WORDS) $this->skip(4);
                    else $this->skip(2);
                    if ($flags & GF_SCALE) $this->skip(2);
                    else 
                        if ($flags & GF_XYSCALE) $this->skip(4);
                        else 
        ...
        // name
        // /////////////////////////////////
        // Needs to have a name entry in 3,0 (e.g. symbol) - original font will
        // be 3,1 (i.e. Unicode)
        ...
        for ($i = 0; $i < $numRecords; $i ++)
        {
            ...
            {
                $pos = 6 + ($i * 12) + 2;
                $name = $this->_set_ushort($name, $pos, 0x00); // Change
                                                               // encoding to
                                                               // 3,0 rather
                                                               // than 3,1
            }
        ...
        {
            $os2 = $this->get_table('OS/2');
            $os2 = $this->_set_ushort($os2, 42, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            $os2 = $this->_set_ushort($os2, 44, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            $os2 = $this->_set_ushort($os2, 46, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            $os2 = $this->_set_ushort($os2, 48, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            
            $os2 = $this->_set_ushort($os2, 50, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            $os2 = $this->_set_ushort($os2, 52, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            $os2 = $this->_set_ushort($os2, 54, 0x00); // ulCharRange (Unicode
                                                       // ranges)
            $os2 = $this->_set_ushort($os2, 56, 0x00); // ulCharRange (Unicode
                                                       // ranges)
                                                       // Set Symbol character
                                                       // only in
                                                       // ulCodePageRange
            $os2 = $this->_set_ushort($os2, 78, 0x8000); // ulCodePageRange =
                                                         // Bit #31 Symbol ****
                                                         // 78 = Bit 16-31
            $os2 = $this->_set_ushort($os2, 80, 0x0000); // ulCodePageRange =
                                                         // Bit #31 Symbol ****
                                                         // 80 = Bit 0-15
            $os2 = $this->_set_ushort($os2, 82, 0x0000); // ulCodePageRange =
                                                         // Bit #32- Symbol ****
                                                         // 82 = Bits 48-63
            $os2 = $this->_set_ushort($os2, 84, 0x0000); // ulCodePageRange =
                                                         // Bit #32- Symbol ****
                                                         // 84 = Bits 32-47
            ...
            $os2 = $this->splice($os2, 32, 
                    chr(5) . chr(0) . chr(1) . chr(0) . chr(1) . chr(0) . chr(0) .
                             chr(0) . chr(0) . chr(0));
            $this->add('OS/2', $os2);
        }
            ...
            &$contours)
    {
        ...
        if (count($this->glyphdata[$originalGlyphIdx]['compGlyphs']))
        {
            ...
            {
                $this->getGlyphData($glyphIdx, $maxdepth, $depth, $points, 
                        $contours);
            }
        }
        else 
            if (($this->glyphdata[$originalGlyphIdx]['nContours'] > 0) &&
                     $depth > 0)
            { // simple
    ...
    function getGlyphs ($originalGlyphIdx, &$start, &$glyphSet, &$subsetglyphs)
    {
        ...
        if ($numberOfContours < 0)
        {
            ...
            while ($flags & GF_MORE)
            {
                ...
                if (! isset($glyphSet[$glyphIdx]))
                {
                    $glyphSet[$glyphIdx] = count($subsetglyphs); // old glyphID
                                                                 // to new
                                                                 // glyphID
    ...
    function getHMTX ($numberOfHMetrics, $numGlyphs, &$glyphToChar, $scale)
    {
        ...
        for ($glyph = 0; $glyph < $numberOfHMetrics; $glyph ++)
        {
            ...
            if (isset($glyphToChar[$glyph]) || $glyph == 0)
            {
                ...
                {
                    $aw = 0;
                } // 1.03 Some (arabic) fonts
                                                    // have -ve values for width
                                                    // although should be
                                                    // unsigned value - comes
                                                    // out as e.g. 65108
                                                    // (intended -50)
                if ($glyph == 0)
                {
    ...
    function getCMAP4 ($unicode_cmap_offset, &$glyphToChar, &$charToGlyph)
    {
        ...
        for ($n = 0; $n < $segCount; $n ++)
        {
            ...
            {
                if ($idRangeOffset[$n] == 0) $glyph = ($unichar + $idDelta[$n]) &
                         0xFFFF;
                else
                {
                    $offset = ($unichar - $startCount[$n]) * 2 +
                             $idRangeOffset[$n];
                    $offset = $idRangeOffset_start + 2 * $n + $offset;
                    if ($offset >= $limit) $glyph = 0;
                    else
                    {
                        $glyph = $this->get_ushort($offset);
                        if ($glyph != 0) $glyph = ($glyph + $idDelta[$n]) &
                                 0xFFFF;
                    }
                }
                $charToGlyph[$unichar] = $glyph;
    ...
    function endTTFile (&$stm)
    {
        ...
        {
            $stm .= (pack("Nnnnn", 0x00010000, $numTables, $searchRange, 
    ...
    function repackageTTF ($file, $TTCfontID = 0, $debug = false)
    {
        ...
        /*
         * Tables which require glyphIndex hdmx kern LTSH Tables which do NOT
         * require glyphIndex VDMX GDEF GPOS GSUB JSTF DSIG PCLT - not
         * recommended
         */
