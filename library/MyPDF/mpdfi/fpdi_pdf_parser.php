...
class fpdi_pdf_parser extends pdf_parser
{
    ...
    function fpdi_pdf_parser ($filename, &$fpdi)
    {
        ...
        $pages = $this->pdf_resolve_object($this->c, 
                $this->root[1][1]['/Pages']);
        if ($this->success == false)
        {
            return false;
    ...
    function getContent ()
    {
        ...
        {
            $contents = $this->_getPageContent(
                    $this->pages[$this->pageno][1][1]['/Contents']);
            ...
            if ($content_ref[0] == PDF_TYPE_ARRAY)
            {
                foreach ($content_ref[1] as $tmp_content_ref)
                {
                    $contents = array_merge($contents, 
                            $this->_getPageContent($tmp_content_ref));
                }
            }
    ...
    function _rebuildContentStream ($obj)
    {
        ...
        foreach ($filters as $_filter)
        {
            switch ($_filter[1])
            {
                ...
                default:
                    $this->fpdi->error(
                            sprintf("Unsupported Filter: %s", $_filter[1]));
            }
        }
        ...
        if (! is_null($box) && $box[0] == PDF_TYPE_ARRAY)
        {
            ...
            else
            {
                return $this->getPageBox(
                        $this->pdf_resolve_object($this->c, 
                                $page[1][1]['/Parent']), $box_index);
            }
    }
