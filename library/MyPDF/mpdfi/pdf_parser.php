...
class pdf_parser
{
    ...
    function pdf_find_root ()
    {
        if ($this->xref['trailer'][1]['/Root'][0] != PDF_TYPE_OBJREF)
        {
            ...
            $this->errormsg = sprintf(
                    "Wrong Type of Root-Element! Must be an indirect reference");
            return false;
        }
    ...
    function pdf_read_xref (&$result, $offset, $start = null, $end = null)
    {
        if (is_null($start) || is_null($end))
        {
            ...
            if (strlen($data) == 0) $data = trim(fgets($this->f, 1024));
        ...
        for (; $start < $end; $start ++)
        {
            $data = ltrim(fread($this->f, 20)); // Spezifications says: 20 bytes
                                                // including newlines
        ...
        if (strlen(trim($data)) == 0) $data = fgets($this->f, 1024);
        ...
        if (preg_match("/trailer/", $data))
        {
            ...
            if (isset($trailer[1]['/Prev']))
            {
                ...
                $result['trailer'][1] = array_merge($result['trailer'][1], 
                        $trailer[1]);
            }
            else
            {
        ...
        else
        {
            ...
            $this->pdf_read_xref($result, null, (int) $data[0], 
        ...
        switch ($token)
        {
            case '<':
                ...
                while (1)
                {
                    ...
                    $result = substr($c->buffer, $c->offset, 
                            $match - $c->offset);
                    $c->offset = $match + 1;
            ...
            case '(':
                ...
                while (1)
                {
                    ...
                    $esc = preg_match('/([\\\\]+)$/', 
                            $tmpresult = substr($c->buffer, $c->offset, 
                                    $match - $c->offset), $m);
            ...
            case "stream":
                ...
                if ($c->buffer[0] == chr(10) || $c->buffer[0] == chr(13)) $e ++;
                if ($c->buffer[1] == chr(10) && $c->buffer[0] != chr(10)) $e ++;
                
                if ($this->actual_obj[1][1]['/Length'][0] == PDF_TYPE_OBJREF)
                {
                    // mPDF 5.0 Removed pass by reference =&
                    $tmp_c = new pdf_context($this->f);
                    $tmp_length = $this->pdf_resolve_object($tmp_c, 
                            $this->actual_obj[1][1]['/Length']);
                    ...
                    if (($tok2 = $this->pdf_read_token($c)) !== false)
                    {
                        if (is_numeric($tok2))
                        {
                            ...
                            if (($tok3 = $this->pdf_read_token($c)) !== false)
                            {
                                ...
                                // If we get to this point, that numeric value
                                // up
                                // there was just a numeric value. Push the
                                // extra
                                // tokens back into the stack and return the
                                // value.
     ...
     * @param object $c
     *            pdf_context
     * @param array $obj_spec
     *            The object-data
     * @param boolean $encapsulate
     *            Must set to true, cause the parsing and fpdi use this method
     *            only without this para
     */
    function pdf_resolve_object (&$c, $obj_spec, $encapsulate = true)
    {
        ...
        if ($obj_spec[0] == PDF_TYPE_OBJREF)
        {
            ...
            if (isset($this->xref['xref'][$obj_spec[1]][$obj_spec[2]]))
            {
                ...
                if ($header[0] != PDF_TYPE_OBJDEC || $header[1] != $obj_spec[1] ||
                         $header[2] != $obj_spec[2])
                {
                    // mPDF 4.0
                    $this->success = false;
                    $this->errormsg = sprintf(
                            "Unable to find object ({$obj_spec[1]}, {$obj_spec[2]}) at expected location");
                    return false;
                }
                ...
                while (1)
                {
                    ...
                    {
                        // in this case the parser coudn't find an endobj so we
                        // break here
                        break;
                    }
