...
/*
 * This script examines your font directory. Point your browser to
 * http://your.domain/your_path_to _mpdf/utils/font_names.php By default this
 * will examine the folder /ttfonts/ (or the default font directory defined by
 * _MPDF_TTFONTPATH. You can optionally define an alternative folder to examine
 * by setting the variable checkdir below (must be a relative path, or
 * filesystem path). You can optionally output just the font samples as a PDF
 * file by setting $pdf=true.
 */
...
foreach ($ff as $f)
{
    $ret = array();
    $isTTC = false;
    if (strtolower(substr($f, - 4, 4)) == '.ttc' ||
             strtolower(substr($f, - 5, 5)) == '.ttcf')
    { // Mac
                                                                                       // ttcf
        $isTTC = true;
...
foreach ($tempfontdata as $fname => $v)
{
    ...
    {
        if (! $pdf) echo 'WARNING - Font file for ' . $fname .
                 ' may be an italic cursive script, or extra-bold etc.<br />';
        if (isset($tempfontdata[$fname]['I']) && $tempfontdata[$fname]['I'])
        {
            ...
            if (isset($tempfontdata[$fname]['B']) && $tempfontdata[$fname]['B'])
            {
                ...
                if (isset($tempfontdata[$fname]['BI']) &&
                         $tempfontdata[$fname]['BI'])
                {
    ...
    {
        if (! $pdf) echo 'INFO - Font file ' . $fname .
                 ' contains characters in Unicode Plane 1 SMP<br />';
        $tempfontdata[$fname]['smp'] = false;
    }
    if (isset($tempfontdata[$fname]['sip']) && $tempfontdata[$fname]['sip'])
    {
        if (! $pdf) echo 'INFO - Font file ' . $fname .
                 ' contains characters in Unicode Plane 2 SIP<br />';
        if (preg_match('/^(.*)-extb/', $fname, $fm))
        {
            if (isset($tempfontdata[($fm[1])]) && $tempfontdata[($fm[1])])
            {
                ...
                if (! $pdf) echo 'INFO - Font file ' . $fname .
                         ' has been defined as a CJK ext-B for ' . ($fm[1]) .
                         '<br />';
            }
            else 
                if (isset($tempfontdata[($fm[1] . '-exta')]) &&
                         $tempfontdata[($fm[1] . '-exta')])
                {
                    $tempfontdata[($fm[1] . '-exta')]['sip-ext'] = $fname;
                    if (! $pdf) echo 'INFO - Font file ' . $fname .
                             ' has been defined as a CJK ext-B for ' .
                             ($fm[1] . '-exta') . '<br />';
                }
        }
...
foreach ($tempfonttrans as $on => $mn)
{
    ...
    $ond = '"' . $on . '"';
    $html .= '<p style="font-family:' . $on . ';">' . $ond .
             ' font is available as ' . $mn;
...
{
    $ond = '"' . $on . '"';
    echo '<div style="font-family:\'' . $on . '\';">' . $ond .
             ' font is available as ' . $mn;
    if (isset($mpdf->fontdata[$mn]['sip-ext']) && $mpdf->fontdata[$mn]['sip-ext'])
    {
