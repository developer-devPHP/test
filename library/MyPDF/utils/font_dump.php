...
/*
 * This script prints out all characters in a TrueType font file to a PDF
 * document. Point your browser to http://your.domain/your_path_to
 * _mpdf/utils/font_dump.php The font file must be located in /ttfonts/ (or the
 * default font directory defined by _MPDF_TTFONTPATH. By default this will
 * examine the font dejavusanscondensed. You can optionally define an
 * alternative font file to examine by setting the variable below (must be a
 * relative path, or filesystem path):
 */
...
$mpdf->WriteHTML(
        '<style>td { border: 0.1mm solid #555555; } body { font-weight: normal; }</style>');
$mpdf->WriteHTML('<h3 style="font-family:' . $font . '">' . strtoupper($font) .
         '</h3>'); // Separate
                                                                                          // Paragraphs
                                                                                          // defined
                                                                                          // by
...
{
    die(
            "Error - Must be able to read font metrics file: " .
                     _MPDF_TTFONTDATAPATH . $font . '.cw.dat');
}
$counter = 0;
...
$lastrange = $range;
// create HTML content
$html .= '<table cellpadding="2" cellspacing="0" style="font-family:' . $font .
         ';text-align:center; border-collapse: collapse; ">';
$html .= '<tr><td colspan="18" style="font-family:helvetica;font-weight:bold">' .
         strtoupper($font) . '</td></tr>';
$html .= '<tr><td colspan="18" style="font-family:helvetica;font-size:8pt;font-weight:bold">' .
...
for ($i = 32; $i < $max; ++ $i)
{
    if (($i > 0) and (($i % 16) == 0))
    {
                ...
                if ($showmissing)
                {
                    ...
                    if ($range && $range == $lastrange)
                    {
                        ...
                        if ($range)
                        {
                            ...
                            $mpdf->WriteHTML($html);
                            $html = '';
                            $html .= '<table cellpadding="2" cellspacing="0" style="font-family:' .
                                     $font .
                                     ';text-align:center; border-collapse: collapse; ">';
                            $html .= '<tr><td colspan="18" style="font-family:helvetica;font-size:8pt;font-weight:bold">' .
                                     strtoupper($range) . ' (U+' . $rangestart .
                                     '-U+' . $rangeend . ')</td></tr>';
                            $html .= '<tr><td></td>';
                            $html .= '<td></td>';
                            for ($k = 0; $k < 16; $k ++)
                            {
                                $html .= '<td><b>-' . sprintf('%X', $k) .
                                         '</b></td>';
                            }
                            $justfinishedblank = false;
                            $justfinishedblankinvalid = false;
        ...
        if ($i > 0 && ($i % 16) == 0 && ($range != $lastrange))
        {
            ...
            $html = '';
            $html .= '<table cellpadding="2" cellspacing="0" style="font-family:' .
                     $font . ';text-align:center; border-collapse: collapse; ">';
            $html .= '<tr><td colspan="18" style="font-family:helvetica;font-size:8pt;font-weight:bold">' .
                     strtoupper($range) . ' (U+' . $rangestart . '-U+' .
                     $rangeend . ')</td></tr>';
            $html .= '<tr><td></td>';
            $html .= '<td></td>';
            for ($k = 0; $k < 16; $k ++)
