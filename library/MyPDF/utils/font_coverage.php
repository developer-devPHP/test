...
/*
 * This script prints out the Unicode coverage of all TrueType font files in
 * your font directory. Point your browser to http://your.domain/your_path_to
 * _mpdf/utils/font_coverage.php By default this will examine the folder
 * /ttfonts/ (or the default font directory defined by _MPDF_TTFONTPATH. You can
 * optionally define an alternative folder to examine by setting the variable
 * below (must be a relative path, or filesystem path):
 */
...
foreach ($unifile as $line)
{
    if (preg_match('/<control>/', $line, $m))
    {
        ...
        if (preg_match('/^([12]{0,1}[0-9A-Za-z]{4});<(.*?), Last>/', $line, $m))
        {
            ...
            if (preg_match('/^([12]{0,1}[0-9A-Za-z]{4});<(.*?), First>/', $line, 
                    $m))
            {
...
for ($urgp = 0; $urgp < $nofgroups; $urgp ++)
{
    ...
    foreach ($unicode_ranges as $urk => $ur)
    {
        if ($urk >= ($urgp * $ningroup) && $urk < (($urgp + 1) * $ningroup))
        {
            ...
            $rangeend = $ur['endhex'];
            $html .= '<td style="font-family:helvetica;font-size:8pt;font-weight:bold;">' .
                    ...
                    if ($uniinrange)
                    {
                        if ($fontinrange)
                        {
                            ...
                            if ($pc == 1)
                            {
                                ...
                                {
                                    $fullcovers[$urk][] = $fname;
                                    $html .= '<td style="background-color: #00FF00;">' .
                                             $str . '</td>';
                                }
                                else 
                                    if ($pc >= 0.9)
                                    {
                                        ...
                                        if ($pc > 0.75)
                                        {
                                            $html .= '<td style="background-color: #00FFAA;">' .
                                                     $str . '</td>';
                                        }
                                        else 
                                            if ($pc > 0.5)
                                            {
                                                $html .= '<td style="background-color: #AAAAFF;">' .
                                                         $str . '</td>';
                                            }
                                            else 
                                                if ($pc > 0.25)
                                                {
                                                    $html .= '<td style="background-color: #FFFFAA;">' .
                                                             $str . '</td>';
                                                }
                                                else
                                                {
                                                    $html .= '<td style="background-color: #FFAAAA;">' .
                                                             $str . '</td>';
                                                }
                        }
                        else
...
foreach ($unicode_ranges as $urk => $ur)
{
    ...
    $html .= '<tr><td style="font-family:helvetica;font-size:8pt;font-weight:bold;' .
             $ext . '">' . strtoupper($range) . ' (U+' . $rangestart . '-U+' .
             $rangeend . ')' . $ext2 . '</td>';
    $arr = $fullcovers[$urk];
    ...
    if (is_array($arr))
    {
        ...
        if (is_array($narr))
        {
            $html .= '<td style="background-color: #AAAAAA;">' .
                     implode(', ', $narr) . ' (>90%)</td></tr>';
        }
        else
        {
