...
// mPDF 5.4
// Using disk to cache table data can reduce memory usage dramatically, but at a
// cost of increased
...
/*
 * Example: $this->bookmarkStyles = array( 0 => array('color'=> array(0,64,128),
 * 'style'=>'B'), 1 => array('color'=> array(128,0,0), 'style'=>''), 2 =>
 * array('color'=> array(0,128,0), 'style'=>'I'), );
 */
...
// When embedding full TTF font files, remakes the font file using only core
// tables
...
// Used to avoid e.g. Arial Unicode MS (perhaps used for substitutions) to ever
// be fully embedded
...
$this->useAdobeCJK = false; // Uses Adobe CJK fonts for CJK languages
                            // default TRUE; only set false if you have defined
                            // some available fonts that support CJK
                            // If true this will not stop use of other CJK fonts
                            // if specified by font-family:
                            // and vice versa i.e. only dictates behaviour when
                            // specified by lang="" incl. AutoFont()
...
$this->printers_info = false; // Adds date and page info for printer when using
                              // @page and "marks:crop;"
...
// Allows automatic character set conversion if "charset=xxx" detected in html
// header (WriteHTML() )
$this->allow_charset_conversion = true;
$this->biDirectional = false; // automatically determine BIDI text in LTR page
$this->autoFontGroupSize = 2; // 1: individual words are spanned; 2: words+; 3:
                              // as big chunks as possible.
$this->useLang = true; // Default changed in mPDF 4.0

$this->useSubstitutions = false; // Substitute missing characters in
                                 // UTF-8(multibyte) documents - from other
                                 // fonts
$this->falseBoldWeight = 5; // Weight for bold text when using an artificial
                            // (outline) bold; value 0 (off) - 10 (rec. max)
...
$this->collapseBlockMargins = true; // Allows top and bottom margins to collapse
                                    // between block elements
$this->progressBar = 0; // Shows progress-bars whilst generating file 0 off, 1
                        // simple, 2 advanced
$this->progbar_heading = 'mPDF file progress';
$this->progbar_altHTML = ''; // Should include <html> and <body> but NOT end
                             // tags
                             // Can incude <head> and link to stylesheet etc.
                             // e.g. '<html><body><p><img src="loading.gif" />
                             // Creating PDF file. Please wait...</p>';

$this->dpi = 96; // To interpret "px" pixel values in HTML/CSS (see img_dpi
                 // below)
                 
// Automatically correct for tags where HTML specifies optional end tags e.g.
                 // P,LI,DD,TD
                 // If you are confident input html is valid XHTML, turning this
                 // off may make it more reliable(?)
$this->allow_html_optional_endtags = true;
$this->ignore_invalid_utf8 = false;
$this->text_input_as_HTML = false; // Converts all entities in Text inputs to
...
$this->PDFX = false; // true=Forces compliance with PDFX-1a spec
                     // Cannot be used with $this->restrictColorSpace (i.e. no
                     // RGB)
$this->PDFXauto = false; // Overrides warnings making changes when possible to
                         // force PDFX1-a compliance
                         
// PDFA1-b Compliant files
$this->PDFA = false; // true=Forces compliance with PDFA-1b spec
                     // Can use with $this->restrictColorSpace=3 (for a CMYK
                     // file)
                     // Any other settings, uses RGB profile
$this->PDFAauto = false; // Overrides warnings making changes when possible to
                         // force PDFA1-b compliance

$this->ICCProfile = ''; // Colour profile OutputIntent
                        // sRGB_IEC61966-2-1 (=default if blank and PDFA), or
                        // other added .icc profile
                        // Must be CMYK for PDFX, or appropriate type for
                        // PDFA(RGB or CMYK)
                        
// When writing a block element with position:fixed and overflow:auto, mPDF
                        // scales it down to fit in the space
                        // by repeatedly rewriting it and making adjustments.
                        // These values give the adjustments used, depending how
                        // far out
                        // the previous guess was. The lower the number, the
                        // quicker it will finish, but the less accurate the fit
                        // may be.
                        // FPR1 is for coarse adjustments, and FPR4 for fine
                        // adjustments when it is getting closer.
$this->incrementFPR1 = 10; // i.e. will alter by 1/[10]th of width and try again
                           // until within closer limits
$this->incrementFPR2 = 20;
$this->incrementFPR3 = 30;
$this->incrementFPR4 = 50; // i.e. will alter by 1/[50]th of width and try again
                           // when it nearly fits
                           
// DEBUGGING & DEVELOPERS
$this->showStats = false;
$this->debug = false;
$this->debugfonts = false; // Checks and reports on errors when parsing TTF
                           // files - adds significantly to processing time
$this->showImageErrors = false;
$this->table_error_report = false; // Die and report error if table is too wide
                                   // to contain whole words
$this->table_error_report_param = ''; // Parameter which can be passed to show
                                      // in error report i.e. chapter number
                                      // being processed//
...
// BOOKMARKS
$this->anchor2Bookmark = 0; // makes <a name=""> into a bookmark as well as
                            // internal link target; 1 = just name; 2 = name
                            // (p.34)
                            
// CSS & STYLES
$this->CSSselectMedia = 'print'; // screen, print, or any other CSS @media type
                               // (not "all")
...
$this->header_line_spacing = 0.25; // spacing between bottom of header and line
                                   // (if present) - function of fontsize
$this->footer_line_spacing = 0.25; // spacing between bottom of header and line
                                   // (if present) - function of fontsize
                                   // If 'pad' margin-top sets fixed distance in
                                   // mm (padding) between bottom of header and
                                   // top of text.
                                   // If 'stretch' margin-top sets a minimum
                                   // distance in mm between top of page and top
                                   // of text, which expands if header is too
                                   // large to fit.
$this->setAutoTopMargin = false;
$this->setAutoBottomMargin = false;
$this->autoMarginPadding = 2; // distance in mm used as padding if 'stretch'
                              // mode is used
                              
// TABLES
$this->simpleTables = false; // Forces all cells to have same border, background
                             // etc. Improves performance
$this->packTableData = false; // Reduce memory usage processing tables (but with
                              // increased processing time)
$this->ignore_table_percents = false;
$this->ignore_table_widths = false;
$this->keep_table_proportions = false; // If table width set > page width, force
                                       // resizing but keep relative sizes
                                       // Also forces respect of cell widths set
                                       // by %
$this->shrink_tables_to_fit = 1.4; // automatically reduce fontsize in table if
                                   // words would have to split ( not in CJK)
                                   // 0 or false to disable; value (if set)
                                   // gives maximum factor to reduce fontsize

$this->tableMinSizePriority = false; // If page-break-inside:avoid but cannot
                                     // fit on full page without
                                     // exceeding autosize; setting this value
                                     // to true will force respsect for
                                     // autosize, and disable the
                                     // page-break-inside:avoid

$this->use_kwt = false;
$this->iterationCounter = false; // Set to TRUE to use table Head iteration
                                 // counter
...
$this->justifyB4br = false; // In justified text, <BR> does not cause the
                            // preceding text to be justified in browsers
                            // Change to true to force justification (as in MS
                            // Word)

$this->tabSpaces = 8; // Number of spaces to replace for a TAB in <pre> sections
                      // Notepad uses 6, HTML specification recommends 8
$this->jSWord = 0.4; // Proportion (/1) of space (when justifying margins) to
                     // allocate to Word vs. Character
$this->jSmaxChar = 2; // Maximum spacing to allocate to character spacing. (0 =
                      // no maximum)

$this->jSmaxCharLast = 1; // Maximum character spacing allowed (carried over)
                          // when finishing a last line
$this->jSmaxWordLast = 2; // Maximum word spacing allowed (carried over) when
                          // finishing a last line
$this->orphansAllowed = 5; // No of SUP or SUB characters to include on line to
                           // avoid leaving e.g. end of line//<sup>32</sup>
$this->normalLineheight = 1.33; // Value used for line-height when CSS specified
                                // as 'normal' (default)
                                
// CJK Line-breaking
$this->allowCJKorphans = true; // FALSE=always wrap to next line; TRUE=squeeze
                               // or overflow
$this->allowCJKoverflow = false; // FALSE=squeeze; TRUE=overflow (only some
                                 // characters, and disabled in tables)
                                 
// HYPHENATION
$this->hyphenate = false;
$this->hyphenateTables = false;
$this->SHYlang = "en"; // Should be one of:
...
// COLUMNS
$this->keepColumns = false; // Set to go to the second column only when the
                            // first is full of text etc.
$this->max_colH_correction = 1.15; // Maximum ratio to adjust column height when
                                   // justifying - too large a value can give
                                   // ugly results
$this->ColGap = 5;

// LISTS
$this->list_align_style = 'R'; // Determines alignment of numbers in numbered
                               // lists
$this->list_indent_first_level = 0; // 1/0 yex/no to indent first level of list
$this->list_number_suffix = '.'; // Content to follow a numbered list marker
                                 // e.g. '.' gives 1. or IV.; ')' gives 1) or a)
...
// Accepts any PDF spec. value: Normal, Multiply, Screen, Overlay, Darken,
// Lighten, ColorDodge, ColorBurn,
// HardLight, SoftLight, Difference, Exclusion
// "Multiply" works well for watermark image on top

// BORDERS
$this->autoPadding = false; // Automatically increases padding in block elements
                            // when border-radius set - if required
...
$this->default_lineheight_correction = 1.2; // Value 1 sets lineheight=fontsize
                                          // height;
                                          // Value used if line-height not set
                                          // by CSS (usuallly is)
...
$this->pregASCIIchars1 = "\x{0021}-\x{002E}\x{0030}-\x{003B}?"; // no [SPACE]
                                                                // Use for
                                                                // words+
$this->pregASCIIchars2 = "\x{0020}-\x{002E}\x{0030}-\x{003B}?"; // [SPACE]
                                                                // punctuation
                                                                // and 0-9
                                                                // Use for
                                                                // chunks >
                                                                // words
$this->pregASCIIchars3 = "\x{0000}-\x{002E}\x{0030}-\x{003B}\x{003F}-\x{007E}"; // all
                                                                                // except
                                                                                // <>
                                                                                // Vietnamese
                                                                                // -
...
// Characters of Urdu, Pashto, Sindhi (but NOT arabic or persian/farsi) [not
// covered by DejavuSans font]
...
// INDIC
$this->pregHIchars = "\x{0900}-\x{0963}\x{0966}-\x{097F}"; // Devanagari (Hindi)
                                                           // minus the common
                                                           // indic punctuation
                                                           // 0964,0965
...
// 200B-D=Zero-width joiners; 0964,0965=Generic Indic punctuation; NBSP &
// general punctuation (excludes # and / so can use in autoFont() )
